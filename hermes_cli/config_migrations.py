...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_hermes_home()}/config.yaml and run `oria setup` to "
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    message = (
        ...
        "Use `oria plugins enable <name>` to activate.")
    _commit(
        config, results, quiet,
        ...
        for label, added in (("curator", added_curator), ("auxiliary.curator", added_aux)):
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added)}) — edit via `oria config set`")
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if changed:
        _commit(
            ...
            "lists — video generation now lives under `oria tools` → "
...
def _migrate_to_45(results: Dict[str, Any], quiet: bool) -> None:
    ...
    _commit(
        ...
        "Uncheck Connections in `oria tools` to turn it off.")
...
MIGRATIONS: Tuple[Tuple[int, Callable[[Dict[str, Any], bool], None]], ...] = (
    ...
    (44, _rewrite_stale_default(
        ...
        message=(
            ...
            "skills/.archive/ (recoverable with `oria curator restore`). Set it back to 90 to keep the old window."))),
