    ...
    commands = [
        ...
        ("setup", "Interactive wizard: install + CA + mint tokens + write config", cmd_setup, [
            ...
            ("--no-restart", dict(dest="restart", action="store_false", help=(
                ...
                "`oria egress restart` yourself for changes to take effect."))),
        ]),
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Start:   [cyan]oria egress start[/cyan]\n"
        "  Restart: [cyan]oria egress restart[/cyan]    (after any re-setup)\n"
        "  Reload:  [cyan]oria egress reload[/cyan]     (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]oria egress status[/cyan]\n"
        "  Stop:    [cyan]oria egress stop[/cyan]\n"
        "  Disable: [cyan]oria egress disable[/cyan]"
    )
    return 0
    ...
    if rotate and existing:
        ...
        if sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Oria sandbox.  They will start 401-ing against upstreams until restarted."
            )
...
def _setup_restart_daemon(console: Console, args: argparse.Namespace, proxy_cfg: dict) -> None:
    ...
    if do_restart:
        try:
            ...
            console.print("  Run [cyan]oria egress start[/cyan] manually before launching new Docker sandboxes.")
        else:
        ...
        console.print(
            ...
            "changed.  Run [cyan]oria egress restart[/cyan] (or "
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print("[yellow]proxy.enabled is false — run `oria egress setup` first.[/yellow]")
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            return _refuse(
                ...
                "back to env credentials with `oria egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            return _refuse(
                ...
                "`oria egress setup --no-bitwarden` to switch back to env-based credentials.",
            )
        if not (bw_cfg or {}).get("project_id"):
            return _refuse(
                console,
                "credential_source=bitwarden but secrets.bitwarden.project_id is empty.",
                "Run `oria secrets bitwarden setup` to configure the "
                "project, or switch back via `oria egress setup --no-bitwarden`.",
            )
    try:
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "still need `oria egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `oria egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `oria egress start` before launching Docker sandboxes."])
    return "\n".join(lines)
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            "  iron-proxy is still running — stop it with [cyan]oria egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print("[yellow](no config generated — run `oria egress setup`)[/yellow]")
        return 1
...
def _bitwarden_env_names(console: Console) -> Optional[List[str]]:
    ...
    if not bw_cfg.get("enabled"):
        ...
        console.print("  Run `oria secrets bitwarden setup` first, or omit --from-bitwarden.")
        return None
    try:
