...
_NO_TOOLS = "the agent will have no tools on this platform. Run `oria tools` to reconfigure."
...
def validate_platform_toolsets(
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `oria tools` to configure explicitly.")
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `oria tools` to reconfigure.")
    return warnings
