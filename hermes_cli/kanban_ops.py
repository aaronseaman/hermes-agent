...
_DAEMON_DEPRECATED = (
    "oria kanban daemon: DEPRECATED — the dispatcher now runs\ninside the gateway. To use "
    "kanban:\n\n    oria gateway start         # starts the gateway + embedded dispatcher\n\nReady "
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                ...
                print(
                    ...
                    f"kanban list --status ready` / `oria kanban list --status blocked` for "
