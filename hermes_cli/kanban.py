...
def _check_dispatcher_presence(hermes_home: Optional[Path] = None) -> tuple[bool, str]:
    ...
    if pid:
        return (False, "Gateway is running but kanban.dispatch_in_gateway=false in "
                ...
                "standalone daemon (`oria kanban daemon --force`).")
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: oria kanban <action> [options]\n"
                  "Run 'oria kanban --help' for the full list of actions.", file=sys.stderr)
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            return _err(f"kanban: board {normed!r} does not exist. "
                        f"Create it with `oria kanban boards create {normed}`.")
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.hermes/profiles/.\n"
              "Create one with `oria -p <name> setup` before assigning tasks.")
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if _json_out(args, data):
        return 0
    if not data:
        print("(no assignees — create a profile with `oria -p <name> setup`)")
        return 0
...
def _cmd_link(args: argparse.Namespace) -> int:
    ...
    if gated:
        print(
            ...
            f"completes; use `oria kanban unlink {args.parent_id} {args.child_id}` "
            f"to run it now."
        )
    return 0
        ...
        def op(tid):
            ...
            try:
                ...
                fail_msg[tid] = (f"cannot complete {tid}: a live worker is running it. Wait for the "
                                 f"worker, `oria kanban reclaim {tid}` to release it, or re-run with "
