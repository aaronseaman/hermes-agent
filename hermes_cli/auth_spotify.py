...
def _refresh_spotify_oauth_state(state: Dict[str, Any], *, timeout_seconds: float = 20.0) -> Dict[str, Any]:
    ...
    if not refresh_token:
        raise _spotify_err(
            "Spotify refresh token missing. Run `oria auth spotify` again.",
    ...
    payload = _spotify_token_post(
        ...
        failed_suffix=" Run `oria auth spotify` again.", relogin_required=True,
    )
...
def resolve_spotify_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise _spotify_err(
                "Spotify is not authenticated. Run `oria auth spotify` first.", "spotify_auth_missing", relogin=True,
            )
    ...
    if not access_token:
        raise _spotify_err(
            "Spotify access token missing. Run `oria auth spotify` again.",
...
def login_spotify_command(args) -> None:
    ...
    print(
        ...
        f"Open this URL to authorize Oria:\n{authorize_url}\n\nFull setup guide: {SPOTIFY_DOCS_URL}\n"
    )
