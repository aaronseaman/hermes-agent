...
_STASH_HINT = "  Stash or commit your changes, then rerun `oria update`."
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    try:
        ...
        print("  Re-run `oria update` to retry; if the agent won't start, reinstall from https://hermes-agent.nousresearch.com")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `oria update "
            f"--branch {branch}`, or update against main with `oria update`."
        )
        _m().sys.exit(1)
    ...
    if not import_ok:
        ...
        print("  Re-run `oria update` to complete it.")
        _m().sys.exit(1)
