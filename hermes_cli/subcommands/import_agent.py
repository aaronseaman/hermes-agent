...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent", help="Import a Claude Code or Codex CLI setup into Oria",
        description="One-command import of another coding agent's setup into Oria. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Oria equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'oria setup' for those.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Overwrite existing Oria items on name conflicts (default: skip)")
