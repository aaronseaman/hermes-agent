...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (oria peer dm <peer>[/<agent>] ...)")
...
def _resolve_peer_target(target: str) -> tuple[str, str | None, dict, str]:
    ...
    if not isinstance(peer, dict) or not peer.get("url"):
        raise LookupError(f"No peer named '{peer_name}'. Run: oria peer list")
    key = _peer_secret(peer_name)
    if not key:
        raise PermissionError(
            f"No API key for peer '{peer_name}'. Set it: oria peer add {peer_name} "
...
def _peer_add(args) -> int:
    ...
    if key:
        ...
        print(
            ...
            f"  oria peer add {name} --url {url} --key <key>\n"
...
def _peer_list(args) -> int:
    peers = _load_peers()
    if not peers:
        print("No peers registered. Add one: oria peer add <name> --url http://host:port --key <API_SERVER_KEY>")
        return 0
...
def cmd_peer(args) -> int:
    ...
    if action not in {"dm", "run", "status", "stop"}:
        print("Unknown peer action. See: oria peer --help", file=sys.stderr)
        return 2
    try:
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer", help="Bot-to-bot DMs across machines (peer Oria gateways)",
        description="Register other Oria gateways as peers and message their agents. "
            "'oria peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'oria -p <bot> chat'. "
        ...
        epilog=(
            ...
            "  oria peer list\n"
            '  oria peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  oria peer dm spark/researcher "..."     # named profile on a multiplexed peer\n'
            "  oria peer run spark --idempotency-key ticket-123 < long-task.txt\n"
            "  oria peer status spark run_abc123\n"
            "  oria peer stop spark run_abc123\n"
            "  oria peer remove spark\n"
            "\n"
    ...
    def _remote(name: str, help: str, *, run_id: bool):
        ...
        if run_id:
            sp.add_argument("run_id", help="Run ID returned by 'oria peer run'")
        else:
