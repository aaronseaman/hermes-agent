...
_CHARGE_ERROR_COPY = {
    ...
    "cli_billing_disabled": "  Remote spending is off for this account — a billing admin can turn it on from the portal's Oria page.",
...
_KILLSWITCH_REASON_OVERVIEW = "A billing admin can turn it on from the portal's Oria page to add funds here."
_KILLSWITCH_REASON_BUY = "A billing admin can turn it on from the portal's Oria page before adding funds."
...
class CLIBillingMixin:
    ...
    def _print_logged_out(self, state, load_failed, cmd) -> None:
        ...
        if state.error:
            ...
            print(f"  Run `oria portal` to log in, then {cmd}.")
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `oria portal` and allow Remote Spending, then re-run /subscription.",
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `oria portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `oria portal` to log in again.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `oria portal` and allow Remote Spending, then retry.",
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Oria page, then run /topup again.")
