...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print("[green]✓ Bitwarden Secrets Manager is enabled.[/green]  "
                  "Secrets will be pulled at the start of every Oria process.")
    console.print("  Status:  [cyan]oria secrets bitwarden status[/cyan]\n"
                  "  Refresh: [cyan]oria secrets bitwarden sync[/cyan]\n"
                  "  Disable: [cyan]oria secrets bitwarden disable[/cyan]")
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]oria secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token:
        console.print(f"\n  [yellow]Enabled but {token_env} is not set — Oria will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    def verify(token: str) -> bool:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `oria secrets bitwarden setup` to pick a different project.[/yellow]")
        return True
    ...
    return rotate_token(
        ...
        disabled_note=None if bw_cfg.get("enabled") else (
            ...
            "run `oria secrets bitwarden setup` (or set "
...
_PROJECT_LIST_HINTS = (
    (("invalid_client", "400 bad request"),
     ...
     "[cyan]oria secrets bitwarden setup[/cyan] and pick EU or "
