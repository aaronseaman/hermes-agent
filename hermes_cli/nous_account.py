...
FREE_TIER_NEEDS_ACCOUNT = "This needs a Nous account. Run `oria auth upgrade`."
...
def format_nous_portal_entitlement_message(
    ...
    if account_info is None:
        return (
            f"Oria could not verify your Nous Portal entitlement, so {capability} is unavailable. "
            f"Run `oria model` to refresh your login, or check billing at {billing_url}."
        )
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Oria cannot verify your Nous Portal "
                f"paid access for {capability}. Log in with `oria model` to enable Portal-managed "
                f"features. Billing and credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `oria model`. "
            f"Billing and credits are managed at {billing_url}."
        )
    if account_info.paid_service_access is None:
        detail = f"Oria could not verify your Nous Portal paid access, so {capability} is unavailable."
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `oria model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Oria could not find a Nous Portal account or organisation for this login, so {capability} "
            f"is unavailable. Run `oria model` to authenticate again; if the problem persists, contact Nous support."
        )
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `oria model` to refresh Oria."
        return message
    return (
...
def _no_paid_access_message(
    ...
    if access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f"member spend cap at {billing_url}, then run `oria model` to refresh."
        )
    if active and paid:
        return (
