...
def _build_full_manifest(
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Oria in threads and DMs."}
        ...
        features["agent_view"] = {"agent_description": "Chat with Oria in Slack Messages."}
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Oria agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    name = getattr(args, "name", None) or "Oria"
    description = getattr(args, "description", None) or "Your Oria agent on Slack"
    ...
    def fail(msg: str) -> int:
        print(f"oria slack manifest: {msg}", file=sys.stderr)
        return 2
    ...
    print(
        "\nNext steps:\n"
        "  1. Open https://api.slack.com/apps and pick your Oria app\n"
