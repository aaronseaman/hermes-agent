...
def run_gui_uninstall(args):
    ...
    print()
    _print_box("│         ☤ Oria Chat GUI Uninstaller                    │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Oria Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Oria agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Oria agent at {hermes_home / 'hermes-agent'}")
    ...
    print()
    print("The Oria agent is still installed. Run 'hermes' to use the CLI,")
    print("or 'oria uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    print()
    _print_box("│            ☤ Oria Uninstaller                          │", Colors.MAGENTA)
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Oria data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Oria code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Oria PATH entries from shell configs / Windows User PATH")
    print("  • Oria wrapper scripts and Hermes-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Oria config/data: {hermes_home}")
    else:
        print(f"  • Oria config/data: {hermes_home}")
...
def _perform_uninstall(
    *,
    ...
    print()
    print("Thank you for using Oria! ☤")
    print()
