...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not path.exists():
        return {}
    fix_hint = "Fix it with `oria config edit` (or move it aside), then re-run the import."
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record("slash-commands", commands_dir, None, "skipped",
                        "Claude slash commands have no direct Oria equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                self.record(kind, name, f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Oria config")
                continue
...
def import_agent_command(args) -> None:
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: oria import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: oria import-agent claude-code   or   oria import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ☤ Oria — Import From Another Agent            │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info(f"Specify a custom path: oria import-agent {agent} --source /path/to/{_AGENT_DEFAULT_DIRS[agent]}")
        return
    ...
    print_info("Secrets:     never imported — run 'oria setup' for credentials")
    ...
    if not args.yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: oria import-agent {agent} --yes")
            return
    ...
    try:
        ...
        print_info("Source registered for sync — re-run 'oria import-agent --sync' "
                   "any time to pull in changes.")
    except OSError as exc:
        logger.warning("Could not update import sync manifest: %s", exc)
    print()
    print_success("Import complete.")
    print_info("API keys and credentials were NOT imported — run 'oria setup' "
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    if stripped := report.get("stripped_secrets"):
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'oria setup' or ~/.hermes/.env.")
        print()
