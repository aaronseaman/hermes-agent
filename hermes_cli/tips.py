...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or oria -c.",
    ...
    "Ctrl+Z suspends Oria to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "oria -c resumes your most recent CLI session. oria -c \"project name\" resumes by title.",
    "oria -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "oria -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "oria chat -t web,terminal enables only specific toolsets for a focused session.",
    "oria chat -s github-pr-workflow preloads a skill at launch.",
    "oria chat -q \"query\" runs a single non-interactive query and exits.",
    "oria chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "oria chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "oria --yolo bypasses all dangerous command approval prompts for the entire session.",
    "oria chat --source telegram tags the session for filtering in oria sessions list.",
    "oria -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "oria doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "oria dump outputs a compact setup summary — great for bug reports.",
    "oria config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "oria config edit opens config.yaml in your default editor.",
    "oria config check scans for missing or stale configuration options.",
    "oria sessions browse opens an interactive session picker with search.",
    "oria sessions stats shows session counts by platform and database size.",
    "oria sessions prune --older-than 30 cleans up old sessions.",
    "oria skills search react --source skills-sh searches the skills.sh public directory.",
    "oria skills check scans installed hub skills for upstream updates.",
    "oria skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "oria skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "oria mcp add github --command npx adds MCP servers from the command line.",
    "oria mcp serve runs Oria itself as an MCP server for other agents.",
    "oria auth add lets you add multiple API keys for credential pool rotation.",
    "oria completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "oria logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "oria backup creates a zip backup of your entire Oria home directory.",
    "oria profile create coder creates an isolated profile that becomes its own command.",
    "oria profile create work --clone copies your current config and keys to a new profile.",
    "oria update syncs new bundled skills to ALL profiles automatically.",
    "oria gateway install sets up Oria as a system service (systemd/launchd).",
    "oria memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "oria webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: oria tools disables unused tools, oria skills config trims skills down.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as oria -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Oria tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'oria profile create coder' creates the 'coder' command.",
    "oria profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Oria prints a resume command with session ID and stats.",
    "oria sessions export backup.jsonl exports all sessions for backup or analysis.",
    "oria -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "oria skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: oria cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Oria runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "oria gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Oria via a local macOS server.",
    ...
    "Oria loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "oria mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "oria mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "oria mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — oria tools can toggle them per platform.",
    ...
    "oria chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "oria plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "oria acp runs Oria as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "oria auth add nous or oria auth add openai-codex sets up OAuth-based providers.",
    ...
    "oria status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Oria your own.",
    ...
    "Before a gateway session reset, Oria auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Oria state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'oria profile create ls' would be rejected.",
    "oria profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "oria claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via oria tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Oria auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Oria config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Oria is working.',
    ...
    # --- Curator ---
    'oria curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "oria curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'oria curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'oria auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    'oria dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'oria dashboard embeds the full Oria TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'oria -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'oria chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'oria chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'oria chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "oria chat --source tool tags programmatic chats so they don't clutter oria sessions list.",
    'oria dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'oria sessions rename <ID> "new title" renames any past session; oria sessions delete <ID> removes one.',
    'oria import restores a full Oria backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'oria fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'oria pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'oria setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'oria status --deep runs the full health sweep across every component; plain oria status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Oria block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from oria sessions list by default — set --source explicitly to see them.',
