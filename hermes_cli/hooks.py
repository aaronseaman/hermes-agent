...
def hooks_command(args) -> None:
    ...
    if not sub:
        print("Usage: oria hooks {list|test|revoke|doctor}")
        print("Run 'oria hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if drift is True:
                        print(
                            ...
                            f"run `oria hooks doctor` to re-validate"
                        )
            print()
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Oria wire shape): {json.dumps(parsed)}")
    else:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if drift is True:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `oria hooks revoke` + re-approve to refresh")
        elif drift is False:
