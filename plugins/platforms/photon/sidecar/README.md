...
Small Node helper that bridges Oria to Photon's Spectrum SDK
(`spectrum-ts`).  Oria is Python; Photon has no public HTTP
...
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Oria can receive inbound messages
...
The Oria plugin's `oria photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Oria is Python, so replies go through this sidecar
