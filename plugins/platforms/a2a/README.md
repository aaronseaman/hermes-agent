# A2A — Agent-to-Agent protocol for Oria
...
peer (another Oria, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
oria gateway setup        # pick A2A, or:
```
...
When the `a2a` platform is enabled, Oria serves a v1.0 Agent Card at
