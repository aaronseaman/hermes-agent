...
│                                                                      │
│   NodeServer (from `oria meet node run`)                             │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `oria meet` CLI |
...
| `cli.py` | `oria meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `oria meet node {run,list,approve,remove,status,ping}` |
...
oria meet install                                        # pip + Chromium
oria meet setup                                          # preflight
oria meet auth                                           # optional
oria meet join https://meet.google.com/abc-defg-hij      # transcribe
```
...
```bash
oria meet install --realtime                       # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hermes/.env
oria meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
oria meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
oria meet install --realtime       # runs: brew install blackhole-2ch ffmpeg
...
oria plugins enable google_meet
oria meet node run --display-name my-mac --host 0.0.0.0 --port 18789
