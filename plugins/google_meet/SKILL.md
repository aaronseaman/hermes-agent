...
oria meet install                   # pip deps + Chromium (transcribe only)
oria meet install --realtime        # + pulseaudio-utils / brew blackhole+ffmpeg
oria meet auth                      # optional; skips guest-lobby wait
oria meet setup                     # preflight checks
```

`oria meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
oria plugins enable google_meet
oria meet node run --display-name my-mac      # persistent server
# copy the printed token

# on the gateway:
oria meet node approve my-mac ws://<mac-ip>:18789 <token>
oria meet node ping my-mac                     # confirm reachable
```

Run `oria meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "An Oria agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `oria meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `oria meet node approve`.
