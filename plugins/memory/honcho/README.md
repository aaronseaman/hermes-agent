...
```bash
oria memory setup honcho     # configure Honcho directly (works on a fresh install)
oria memory setup            # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
oria config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hermes/.env
```

> `oria honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `oria memory setup honcho`.
...
`oria honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `oria model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated Oria instances) |
| 2 | `~/.hermes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Oria profile: `oria` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `oria honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
- **`per-session`** — Oria session ID (timestamp + hex). Every `oria` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Oria profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`oria`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Oria profile: `oria` (default) or `hermes_<profile>` (e.g. `oria -p coder` -> host key `hermes_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `oria memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `oria honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `oria memory setup`) |
| `oria honcho status` | Show resolved config for active profile |
| `oria honcho enable` / `disable` | Toggle Honcho for active profile |
| `oria honcho mode <mode>` | Change recall or observation mode |
| `oria honcho peer --user <name>` | Update user peer name |
| `oria honcho peer --ai <name>` | Update AI peer name |
| `oria honcho tokens --context <N>` | Set context token budget |
| `oria honcho tokens --dialectic <N>` | Set dialectic max chars |
| `oria honcho map <name>` | Map current directory to a session name |
| `oria honcho sync` | Create host blocks for all Oria profiles |
