...
This plugin ships bundled with Oria but is **opt-in** — it only loads when
...
oria tools    # → Langfuse Observability
...
Set these in `~/.hermes/.env` (or via `oria tools`):
...
oria chat -q "hello"              # then check Langfuse for a "Oria turn" trace
```

Generation observations include the Oria system prompt when the provider
...
```bash
oria plugins disable observability/langfuse
```
