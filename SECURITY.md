# Oria Security Policy

This document describes Oria's trust model, names the one
...
security vulnerabilities. **Oria does not operate a bug
bounty program.**
...
- Environment details (`oria --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Oria access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Oria's documentation
...
Oria supports two OS-level isolation postures. They address
...
Oria supports this in two ways:

- **Oria's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Oria's in-process heuristics
...
Oria filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Oria
itself. Bugs in Oria's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Oria does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Oria's own documentation, or reasonable
  operator expectations would predict — including cases where
  Oria has documented a stance about how its output should
...
- Oria includes supply-chain guards for MCP server
