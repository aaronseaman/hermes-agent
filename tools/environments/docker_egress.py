...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        return _degraded(
            ...
            "Re-run `oria egress setup` to regenerate it.")
    ...
    if not mappings:
        return _degraded(
            ...
            "corrupt.  Re-run `oria egress setup` to mint provider "
