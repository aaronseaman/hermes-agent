# Contributing to Oria

Thank you for contributing to Oria! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Oria to work everywhere.
...
Related: #38284 covers the agent-side analog — Oria itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Oria install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `oria skills browse` (labeled "official") and install it with `oria skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `oria skills install`.
...
- Integrate with `oria memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Oria ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.hermes/plugins/`, project `.hermes/plugins/`, or a pip entry point) — see [Build an Oria Plugin](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin)
...
The installer creates the Oria venv, wires the `oria` command, stamps the
install method for `oria update`, and clones the full git project into
...
```bash
# The standard installer already put `oria` on PATH.
oria doctor
oria chat -q "Hello"
```
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `oria sessions export` for an explicit export.
...
The user may skip setup and keep loading the skill. Oria only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `oria setup` or update `~/.hermes/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Oria tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Oria" is the secondary collaborator. If the contributor's commit shows "Oria" as author because they used Oria to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Oria tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `oria --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Oria uses a data-driven skin system — no code changes needed to add a new skin.
...
Oria runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Oria has terminal access. Security matters.
...
2. **Test manually**: Run `oria` and exercise the code path you changed
...
- Include: OS, Python version, Oria version (`oria --version`), full error traceback
