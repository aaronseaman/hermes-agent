...
export const BACKEND_RESTARTING =
  'Oria stopped unexpectedly — restarting and reopening your chat (the reply in progress was lost).'

export const BACKEND_RESTARTING_ACTIVITY = 'Oria stopped unexpectedly · restarting…'
...
export const CONNECTION_LOST = 'Connection to Oria lost — reconnecting and reopening your chat…'
...
export const backendGaveUp = (code: null | number, lastLine?: string): string => {
  ...
  return [
    `Oria stopped${exit} and could not be restarted. Your chat is saved.`,
    detail,
    'Oria keeps trying to reconnect in the background and reopens this chat when it succeeds; if it does not, type /resume.',
    'Type /logs for the full log, or /quit and run `oria doctor` to check the install.'
  ]
    .filter(Boolean)
    .join('\n')
}

export const BACKEND_GAVE_UP_ACTIVITY = 'Oria stopped · /logs for details'
...
export const BACKEND_SLOW_START =
  'Oria is taking longer than usual to start. Still waiting… If it never connects: /logs shows the last backend output; /quit and run `oria doctor`.'
...
export const stderrProblemActivity = (line: string): string => {
  ...
  return `Something went wrong inside Oria${what} · /logs for details`
}
...
export const VERSION_SKEW_MESSAGE =
  'The terminal UI and the Oria backend are out of sync (different versions). Run /update, or exit and run `oria update`, then start the TUI again.'
...
const RPC_ERROR_ROWS: RpcErrorRow[] = [
  ...
  [
    ...
    () =>
      'Oria is not connected right now, so that was not sent. It reconnects automatically; wait a moment and try again, or type /logs if this persists.'
  ],
  [
    (_code, text) => TIMED_OUT_RE.exec(text),
    m => `Oria did not answer within ${m?.[1] ?? '?'}s. Try again; if it keeps happening, type /logs and report the last lines.`
  ]
]
...
const TURN_LAYER_COPY: Record<string, [string, string]> = {
  ...
  billing: ['The model provider reports no credit left', 'Top up the account or switch with /model.'],
  disk: ['The disk is full, so Oria could not save the turn', 'Free some space, then /retry.'],
  endpoint: ['Your custom model endpoint did not answer', 'Check the endpoint is running, then /retry.'],
  gateway: ['Oria hit an internal error while running this turn', 'Send /retry; type /logs for the trace.'],
