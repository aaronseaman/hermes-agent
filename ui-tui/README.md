...
React + Ink terminal UI for Oria. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- `src/app/setupHandoff.ts` — launches external `oria setup`, suspends Ink while it runs, opens a new session on success
...
### Setup (`setup.ts`)
`/setup` — launches external `oria setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external oria setup, suspends Ink while it runs
