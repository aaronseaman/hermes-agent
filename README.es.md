...
> **Nota:** En Windows nativo, Oria funciona sin WSL — la CLI, el gateway, la TUI y las herramientas funcionan de forma nativa. Si prefieres usar WSL2, el comando de Linux/macOS de arriba también funciona allí. ¿Encontraste un error? Por favor [crea un issue](https://github.com/NousResearch/hermes-agent/issues).
...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\hermes\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). Oria usa este Git Bash incluido para ejecutar comandos de shell.
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://hermes-agent.nousresearch.com/docs/getting-started/termux). En Termux, Oria instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
...
```bash
oria              # CLI interactiva — inicia una conversación
oria model          # Elige tu proveedor y modelo LLM
oria tools          # Configura qué herramientas están habilitadas
oria config set     # Establece valores de configuración individuales
oria gateway        # Inicia el gateway de mensajería (Telegram, Discord, etc.)
oria setup          # Ejecuta el asistente de configuración completo
oria claw migrate   # Migra desde OpenClaw (si vienes de OpenClaw)
oria update         # Actualiza a la última versión
oria doctor         # Diagnostica cualquier problema
```
...
Oria funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Esto te autentica vía OAuth, establece Nous como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `oria portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway).
...
Oria tiene dos puntos de entrada: inicia la interfaz de terminal con `oria`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `oria`                                      | Ejecuta `oria gateway setup` + `oria gateway start`, luego envía un mensaje al bot |
...
Si vienes de OpenClaw, Oria puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`oria setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
oria claw migrate                # Migración interactiva (preset completo)
oria claw migrate --dry-run      # Vista previa de qué se migraría
oria claw migrate --preset user-data     # Migrar sin secretos
oria claw migrate --overwrite    # Sobreescribir conflictos existentes
```
...
Consulta `oria claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Servidor MCP de control de escritorio Linux para Oria y otros hosts MCP, con árboles de accesibilidad AT-SPI, entrada Wayland/X11, capturas de pantalla y targeting de ventanas del compositor.
- 🔌 [HermesClaw](https://github.com/AaronWong1999/hermesclaw) — Puente WeChat comunitario: Ejecuta Oria y OpenClaw en la misma cuenta de WeChat.
