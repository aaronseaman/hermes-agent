# Política de Seguridad de Oria

Este documento describe el modelo de confianza de Oria, identifica el
...
vulnerabilidades de seguridad. **Oria no opera un programa de
...
- Detalles del entorno (`oria --version`, SHA del commit, SO, versión de Python).
...
Oria es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Oria,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso an Oria al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Oria
...
Oria admite dos posturas de aislamiento a nivel de SO. Abordan
...
Oria admite esto de dos maneras:

- **La propia imagen Docker de Oria y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Oria
...
Oria filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Oria
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Oria
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Oria no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Oria o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Oria ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Oria
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Oria. Los errores en la ruta de
  instalación de habilidades o plugins de Oria que impidan al
...
- Oria incluye guardias de cadena de suministro para lanzamientos de servidores
