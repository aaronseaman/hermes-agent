# Themes / Skins — Author an Oria Color Theme

Author an Oria **skin** — one YAML file that themes the CLI, the TUI, and the
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Oria itself.
...
- Write access to the Oria home dir — `~/.hermes` by default, or `$HERMES_HOME`
