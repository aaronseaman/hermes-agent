...
Write plugins for the Oria desktop app: statusbar items, layout panes,
...
- The Oria desktop app (it loads plugins; the CLI/gateway alone does not).
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Oria
