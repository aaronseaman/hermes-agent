...
## Layer 1 — Is this thing an Oria plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Oria process**. Its LLM calls go through Oria's already-configured provider. | Already uses Portal if user's Oria is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Oria. That LLM call is already authenticated
via Oria's provider config. The plugin's own server auth (e.g.
...
1. Read Oria's existing Portal credential out of `~/.hermes/auth.json`.
...
rides on the credential Oria already holds.

Where this could live in Oria:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Oria itself.
...
1. First decide: Oria plugin (runs inside Oria) or separate app? If plugin,
   it already uses Portal via Oria's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Oria agent itself") is fully supported. Oria treats agent
...
pitch in the Tool Gateway announcement is specifically about Oria's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Oria is configured to use Portal as its provider. It is
