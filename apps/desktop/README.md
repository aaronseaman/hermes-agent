...
**The native desktop app for [Oria](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Oria surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Oria and hear it back.</td></tr>
...
### Install with Oria (recommended)

Already have the Oria CLI? Just run:

```bash
oria desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Oria gateway or install Oria locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Oria Desktop website.](https://hermes-agent.nousresearch.com/).
...
first launch it can install the Oria runtime into `HERMES_HOME`
...
- **Oria** runs as a headless `oria serve` process and exposes the
...
4. `HERMES_DESKTOP_HERMES`, or `oria` on `PATH`
5. a system Python that can import the Oria runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Oria
...
screen offers **Connect to existing Oria** before starting the local installer.
...
terminal commands, and file operations run against the remote Oria host, not
...
> The default Oria home on Windows is `%LOCALAPPDATA%\hermes`. Set the `HERMES_HOME` env var if you've relocated it.
