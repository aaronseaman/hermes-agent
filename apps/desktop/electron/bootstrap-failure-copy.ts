...
const BOOTSTRAP_FAILURE_REMEDY =
  'Common causes: no internet connection, antivirus blocking the installer, or another copy of Oria running. ' +
  'Close other Oria windows and choose Reload and retry; if it fails again, open the logs and send them to support.'
...
export function describeBootstrapFailure(failedStage: string | null | undefined, rawError: unknown): string {
  ...
  const lead = label
    ? `Setting up Oria stopped during the '${label}' step.`
    : 'Setting up Oria stopped before it could finish.'
...
export function missingInstallPartMessage(whatIsMissing: string): string {
  return (
    "Part of Oria's installation is missing (it may have been deleted or quarantined by antivirus). " +
