...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Oria process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the Oria installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other Oria windows and terminals, then retry.  If the problem\n' +
    'persists, run `oria update` in a terminal for detailed diagnostics.'
  )
}
