...
describe('isProviderSetupErrorMessage', () => {
  it('matches generic missing-provider copy', () => {
    expect(isProviderSetupErrorMessage('No inference provider configured. Run `oria model` to choose one.')).toBe(
      true
    )
    expect(isProviderSetupErrorMessage('No inference provider is configured.')).toBe(true)
    expect(isProviderSetupErrorMessage('No Oria provider is configured.')).toBe(true)
    // Backends from before the Oria rename still word it with the old product name.
  ...
  it('does not match bare env var mentions from auxiliary warnings', () => {
    ...
    expect(isProviderSetupErrorMessage('Run `oria setup` or set OPENROUTER_API_KEY.')).toBe(false)
    expect(
      isProviderSetupErrorMessage(
        '⚠ No auxiliary LLM provider configured — context compression will drop middle turns without a summary. Run `oria setup` or set OPENROUTER_API_KEY.'
      )
    ).toBe(false)
