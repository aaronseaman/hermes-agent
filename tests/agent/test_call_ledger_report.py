...
def test_repeat_rates_carry_the_fingerprint_kinds_they_were_computed_from():
    """A later state-aware fingerprint replaces the field; a slice spanning both says so."""
    from agent.call_ledger_report import format_report, summarize

    assert summarize(RECORDS)["signature_kinds"] == ["tool+canonical_args"]
    mixed = RECORDS + [_tool("t3", "s3", sig="c", idempotent=True, repeated_turn=False,
                             repeated_session=False, duration=0.1, signature_kind="tool+state")]
    assert summarize(mixed)["signature_kinds"] == ["tool+canonical_args", "tool+state"]
    assert "MIXED" in format_report(build_report(mixed), sources=[], per_session=False)
    assert "MIXED" not in format_report(build_report(RECORDS), sources=[], per_session=False)
