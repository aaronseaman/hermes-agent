...
def _script_turn(agent, first: Path, second: Path):
    """3 model calls, 3 tool calls: a batch of two distinct reads, then the first one again."""
    agent.client.chat.completions.create.side_effect = [
        _response(tool_calls=[_read_call("c1", first), _read_call("c2", second)], usage=_usage(1000, 0, 40)),
        _response(tool_calls=[_read_call("c3", first)], usage=_usage(1400, 900, 30)),
...
def test_turn_records_match_calls_and_never_store_raw_args(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)  # the tool env snapshots the CWD; keep it small and hermetic
    home = get_hermes_home()
    _enable_ledger(home)
    first = tmp_path / f"notes-{SECRET}.txt"
    second = tmp_path / "other.txt"
    first.write_text("hello\n", encoding="utf-8")
    second.write_text("world\n", encoding="utf-8")
    agent = _agent()
    _script_turn(agent, first, second)
    try:
        result = agent.run_conversation("read the notes")
        model_call_count = agent.client.chat.completions.create.call_count
    finally:
        agent.close()
    ...
    assert len(model) == model_call_count
    assert len(tools) == sum(1 for m in result["messages"] if m.get("role") == "tool") == 3
    assert len(turns) == 1
    turn = turns[0]
    ...
    # Two distinct (tool, args) pairs; only the third call repeats one of them.
    assert len({t["signature"] for t in tools}) == 2
    assert [t["repeated_in_turn"] for t in tools].count(True) == 1
    assert all(t["effects"]["idempotent"] for t in tools)
    # The batch of two ran as one parallel segment; the lone follow-up call did not.
    assert [t["segment"] for t in tools] == ["parallel", "parallel", "sequential"]
    assert all(t["result_chars"] > 0 and t["duration_s"] >= 0 for t in tools)
...
def test_ledger_failure_does_not_change_the_turn(tmp_path, monkeypatch, breakage):
    ...
    home = get_hermes_home()
    first, second = tmp_path / "notes.txt", tmp_path / "other.txt"
    first.write_text("hello\n", encoding="utf-8")
    second.write_text("world\n", encoding="utf-8")
