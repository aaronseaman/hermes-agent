...
def test_repeated_read_only_procedure_is_compiled_run_without_a_model_and_falls_back_on_drift(
    ...
    # Mined, compiled through skill_manage, replay-validated and activated by a real curator run.
    from agent import curator
    run = curator.run_curator_review(synchronous=True, consolidate=False)
    assert "1 compiled skill(s) compiled" in run["summary_so_far"], run
    assert "1 compiled skill(s) activated" in run["summary_so_far"], run
...
def test_skill_write_approval_gate_stages_the_compiled_skill_and_nothing_activates(workspace, home, no_model):
    with open(home / "config.yaml", "a", encoding="utf-8") as fh:
        fh.write("skills:\n  write_approval: true\n")
    _record_three_sessions(workspace)
    counts = compiled_skill.curator_pass()
    assert counts.get("staged") == 1 and not counts.get("created") and not counts.get("activated"), counts
    assert compiled_skill.compiled_skill_dirs() == {}
    # The staged write is not re-staged by the next pass.
    assert not compiled_skill.curator_pass().get("staged")
