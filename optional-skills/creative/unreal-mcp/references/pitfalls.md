...
### 1. Start order: editor first, Oria second

Oria probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Oria session.
...
restart the editor, restart the Oria session.
...
explicitly warns against overlapping calls. Oria executes same-turn tool
...
`<Project>/Saved/Screenshots/...`). If Oria runs elsewhere (SSH backend,
