...
the actual `setup.sh` that creates the project workspace, configures Oria
...
PyYAML must be installed in the user's Python (it ships with most Oria
...
the Oria `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
