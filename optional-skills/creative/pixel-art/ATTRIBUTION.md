...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Oria uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Oria has `image_generate`
...
- Expansion with palettes + video: Oria contributors
