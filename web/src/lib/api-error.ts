...
export const API_UNREACHABLE_MESSAGE =
  "Oria dashboard cannot reach the Oria service. Is `oria dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The Oria service hit an internal error.",
  502: "The dashboard proxy could not reach the Oria service.",
  503: "The Oria service is not ready yet. Try again in a moment.",
  504: "The Oria service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The Oria service answered with an unexpected error (${status}).`;
}
