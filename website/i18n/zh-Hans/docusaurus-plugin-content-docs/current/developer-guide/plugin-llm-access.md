...
* **宿主持有凭据。** OAuth token、刷新流程、凭据池、每任务辅助覆盖——Oria 已有的所有凭据概念均适用。Plugin 永远看不到 token；宿主通过 `result.audit` 将调用归因回溯。
...
result = ctx.llm.complete(
    ...
    provider=None,         # 可选，受门控——Oria provider id（如 "openrouter"）
...
Plugin 可以为自己注册的任务提供 provider/模型注册默认值。运营人员在 `auxiliary.<task>` 中的配置会覆盖这些默认值，并控制部署选择。Plugin 只能使用自己注册的任务；未知或属于其他 plugin 的任务名会在调用 provider 前失败。`allow_task_override: true` 是运营人员显式授予的权限，用于使用 Oria 内置辅助任务；它不允许使用其他 plugin 的任务。省略 `task=`（或使用 `"auto"`）会继续使用当前主 provider/模型。
...
plugins:
  entries:
    my-plugin:
      llm:
        # 允许此 plugin 选择不同的 Oria provider
        # （必须是 Oria 已知的 provider——与
        # `oria model` 和 config.yaml model.provider 中的名称相同）
...
* **回退链。** 若用户主 provider 返回 5xx 或 429，请求在向 plugin 返回错误前会经过 Oria 常规的聚合器感知回退流程。
