...
你也可以让 Oria 指向**外部 skill 目录**——与本地目录一起扫描的额外文件夹。参见下方的[外部 Skill 目录](#external-skill-directories)。
...
（计划模式的工作方式相同，但现在是内置命令：`/plan [request]` 告知 Oria 在需要时检查上下文、编写 markdown 实现计划而非直接执行任务，并将结果保存在相对于当前工作区/后端工作目录的 `.hermes/plans/` 下。）
...
当遇到缺失的值时，Oria 仅在本地 CLI 中实际加载 skill 时才会安全地请求输入。你可以跳过设置并继续使用该 skill。消息平台不会在聊天中请求密钥——它们会告诉你改用本地的 `oria setup` 或 `~/.hermes/.env`。
...
设置存储在 config.yaml 的 `skills.config` 下。`oria config migrate` 会提示配置未设置的项，`oria config show` 会显示它们。当 skill 加载时，其解析后的配置值会注入到上下文中，agent 会自动知晓已配置的值。
...
通过第三方 URL 或 GitHub 安装时，Oria 会安装 `SKILL.md`，以及其中明确引用且位于 `references/`、`templates/`、`scripts/`、`assets/` 和 `examples/` 下的文件。未引用的仓库文件不会被复制。Oria 会扫描完整的隔离捆绑包，并在 `skills/.hub/lock.json` 中记录来源 URL、精确内容哈希、扫描器版本、发现项、时间戳，以及本次结果是新扫描还是缓存复用。

## 外部 Skill 目录

如果你在 Oria 之外维护 skills——例如，供多个 AI 工具使用的共享 `~/.agents/skills/` 目录——你可以告诉 Oria 也扫描这些目录。
...
- **外部目录不是写保护边界**：如果外部 skill 目录对 Oria 进程可写，agent 管理的 skill 更新可以修改该目录中的文件。如果共享的外部 skills 必须保持只读，请使用文件系统权限或单独的 profile/toolset 设置。
...
- **不存在的路径会被静默跳过**：如果配置的目录不存在，Oria 会忽略它而不报错。适用于可能不在每台机器上都存在的可选共享目录。
...
- `description`（可选）——在 `/bundles` 和 `oria bundles list` 中显示的简短文本。
...
| 来源 | 示例 | 说明 |
|--------|---------|-------|
| `official` | `official/security/1password` | Oria 随附的可选 skills。 |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | 可通过 `oria skills search <query> --source skills-sh` 搜索。当 skills.sh slug 与仓库文件夹不同时，Oria 会解析别名式 skills。 |
...
Oria 目前与以下 skills 生态系统和发现来源集成：

#### 1. 官方可选 skills（`official`）

这些 skills 在 Oria 仓库中维护，以内置信任级别安装。
...
这是 Vercel 的公共 skills 目录。Oria 可以直接搜索它、查看 skill 详情页、解析别名式 slug，并从底层源仓库安装。
...
Oria 可以直接从 GitHub 仓库和基于 GitHub 的 tap 安装。当你已知仓库/路径或想添加自己的自定义源仓库时非常有用。
...
Oria 可以从 LobeHub 的公共目录中搜索并将 agent 条目转换为可安装的 Oria skills。
...
Oria 与 [browse.sh](https://browse.sh) 集成，这是 Browserbase 的目录，包含 200+ 个针对特定站点的浏览器自动化 SKILL.md 文件（Airbnb、Amazon、arXiv、12306.cn、Etsy、Xero 等）。每个 skill 描述如何端到端驱动一个网站，适合与 Oria 的浏览器工具以及你已安装的任何浏览器自动化 skills 配合使用。

- 站点：[browse.sh](https://browse.sh/)
- 目录 API：`https://browse.sh/api/skills`
- Oria 来源 id：`browse-sh`
- 信任级别：`community`
...
直接从任何 HTTP(S) URL 安装 `SKILL.md`——当作者在自己的站点上托管 skill 时非常有用（无 hub 列表，无需输入 GitHub 路径）。Oria 还会获取其中明确引用且位于 `references/`、`templates/`、`scripts/`、`assets/` 和 `examples/` 下的文件，然后扫描并安装完整捆绑包。

- Oria 来源 id：`url`
- 标识符：URL 本身（无需前缀）
- 范围：`SKILL.md` 加上允许目录中明确引用的支持文件。Oria 不会枚举或复制托管站点上的其他文件。
...
信任级别始终为 `community`——与所有其他来源一样运行相同的安全扫描。URL 作为安装标识符存储，因此当你想刷新时，`oria skills update` 会自动从同一 URL 重新获取。

### 安全扫描与 `--force`

所有通过 hub 安装的 skills 都经过**安全扫描器**检查，检测数据泄露、prompt 注入、破坏性命令、供应链信号及其他威胁。

`oria skills inspect ...` 现在还会在可用时显示上游元数据：
- 仓库 URL
- skills.sh 详情页 URL
- 安装命令
- 每周安装量
...
如果你想分享一组精选的 skills——为你的团队、组织或公开分享——你可以将它们发布为 **tap**：其他 Oria 用户通过 `oria skills tap add <owner/repo>` 添加的 GitHub 仓库。无需服务器，无需注册表注册，无需发布流水线。只需一个包含 `SKILL.md` 文件的目录。
...
Oria 通过列出 tap 路径的每个子目录并探测每个目录中的 `SKILL.md` 来发现 skills。
...
将其推送到 GitHub 后，任何 Oria 用户都可以订阅并安装：
...
`oria skills tap add` CLI 默认将新 tap 的 `path` 设为 `"skills/"`；如果需要不同路径，请直接编辑该文件。`oria skills tap list` 显示每个 tap 的有效路径。
...
新 tap 默认分配 `community` 信任级别。从中安装的 skills 经过标准安全扫描，首次安装时显示第三方警告面板。如果你的组织或广泛受信任的来源应获得更高信任，请将其仓库添加到 `tools/skills_hub.py` 中的 `TRUSTED_REPOS`（需要 Oria 核心 PR）。
...
## 捆绑 skill 更新（`oria skills reset`）

Oria 在仓库的 `skills/` 中附带一组捆绑 skills。在安装时以及每次 `oria update` 时，同步过程会将这些 skills 复制到 `~/.hermes/skills/` 中，并在 `~/.hermes/skills/.bundled_manifest` 记录一个清单，将每个 skill 名称映射到同步时的内容哈希（**origin hash**）。

每次同步时，Oria 重新计算本地副本的哈希并与 origin hash 比较：
...
:::note Profiles
每个 profile 在其自己的 `HERMES_HOME` 下有自己的 `.bundled_manifest`，因此 `oria -p coder skills reset <name>` 只影响该 profile。
:::
