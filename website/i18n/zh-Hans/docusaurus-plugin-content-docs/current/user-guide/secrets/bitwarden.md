...
2. Oria 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.hermes/.env` 中。
3. 每次 `oria`（或 gateway，或 cron 任务）启动时，在加载 `~/.hermes/.env` 之后，Oria 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
4. 默认情况下，Oria **覆盖**环境中已有的值，因此 Bitwarden 是唯一可信来源——在 Web 应用中轮换一次密钥，每个 Oria 进程在下次启动时即可获取最新值。如果希望 `.env` 优先，可在配置中将 `override_existing: false`。
...
2. 创建或选择一个**项目**（例如"Oria keys"）。
3. 将提供商密钥添加为 secret。secret 的**名称**将成为环境变量名——使用 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等。
4. **Machine accounts → New machine account → My Oria machine** → **Projects** 标签页 → 授予对你的项目的 Read 权限。
...
此后，每次调用 `oria` 都会在启动时拉取最新 secret。进程中首次应用 secret 时，stderr 会显示一行摘要信息。

## CLI

| 命令 | 功能 |
|---|---|
| `oria secrets bitwarden setup` | 交互式向导（安装二进制文件、提示输入令牌、选择项目、测试拉取） |
| `oria secrets bitwarden status` | 显示配置、二进制版本，以及令牌是否存在/是否通过校验 |
| `oria secrets bitwarden token` | 轮换访问令牌：先向 Bitwarden 验证新令牌，验证通过后再写入 `.env` |
| `oria secrets bitwarden sync` | 演习模式：立即拉取 secret 并显示将应用的内容 |
| `oria secrets bitwarden sync --apply` | 拉取并导出到当前 shell 的环境中 |
| `oria secrets bitwarden install` | 仅下载固定版本的 `bws` 二进制文件（无需认证） |
| `oria secrets bitwarden disable` | 将 `enabled` 设为 `false`；保留令牌和项目 ID |
...
```bash
oria secrets bitwarden token                       # 隐藏输入提示
oria secrets bitwarden token --access-token 0.…    # 非交互式
```
...
| `cache_ttl_seconds` | `300` | 进程内拉取结果的复用时长。设为 `0` 可禁用缓存。缓存按进程隔离；新的 `oria` 调用从头开始。 |
...
Bitwarden 永远不会阻塞 Oria 启动。如果出现任何问题，stderr 会显示一行警告，Oria 继续使用 `.env` 中已有的凭据：

| 现象 | 原因 | 修复方法 |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | 配置中已启用，但令牌已从 `.env` 中清除 | 重新运行 `oria secrets bitwarden setup` |
| `Bitwarden rejected the machine-account access token … invalid_client` | 令牌已吊销、过期、机器账户被删除——或令牌属于其他区域（例如欧盟令牌访问了美国 identity 端点） | 运行 `oria secrets bitwarden token` 粘贴新令牌；区域不匹配时重新运行 setup 选择欧盟/自托管（或设置 `secrets.bitwarden.server_url`） |
| `bws exited 1: invalid access token` | 令牌已吊销或有误 | 运行 `oria secrets bitwarden token` 提供新令牌 |
...
- 即使 `override_existing: true`，Oria 也会拒绝让 Bitwarden 覆盖引导令牌本身。如果你将 `BWS_ACCESS_TOKEN` 作为 secret 存储在项目中，应用时会静默跳过。
- `bws` 二进制文件的下载会与同一 GitHub release 中发布的 SHA-256 校验和进行验证。不匹配时将中止安装。
- 固定版本（撰写本文时为 `bws v2.0.0`）通过向本仓库提交 PR 的方式更新——Oria 不会将 `bws` 自动升级到"最新版本"，因为上游 release 的结构可能发生变化。
...
适合使用此功能的场景：多机器集群、共享开发机、gateway VPS，或任何需要跨多个 Oria 安装进行集中轮换和吊销管理的场景。
