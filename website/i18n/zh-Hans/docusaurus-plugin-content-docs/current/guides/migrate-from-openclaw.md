...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 配置迁移到 Oria 的完整指南——包括迁移内容、配置键映射及迁移后的检查事项。"
---

# 从 OpenClaw 迁移

`oria claw migrate` 将你的 OpenClaw（或旧版 Clawdbot/Moldbot）配置导入 Oria。本指南详细说明迁移内容、配置键映射以及迁移后的验证步骤。
...
| `--overwrite` | 冲突时覆盖已有 Oria 文件（默认：计划存在冲突时拒绝执行）。 |
| `--migrate-secrets` | 包含 API 密钥。即使使用 `--preset full` 也需要显式指定——没有任何预设会静默导入密钥。 |
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的 zip 快照备份（默认在执行前写入单个还原点归档，位于 `~/.hermes/backups/pre-migration-*.zip`；可通过 `oria import` 还原）。 |
...
Skill 冲突由 `--skill-conflict` 处理：`skip` 保留现有 Oria skill，`overwrite` 替换，`rename` 创建带 `-imported` 后缀的副本。

### 模型与 Provider 配置

| 内容 | OpenClaw 配置路径 | Oria 目标 | 备注 |
|------|---------------------|-------------------|-------|
...
| 内容 | OpenClaw 配置路径 | Oria 配置路径 | 映射规则 |
|------|---------------------|-------------------|---------|
...
不会导入空闲或每日重置计时器。Oria 会话会持续保留，直到显式使用 `/new` 或 `/reset`。身份关联、线程绑定、维护、作用域和发送策略等高级设置仍会归档以供参考。

### MCP 服务器

| OpenClaw 字段 | Oria 字段 | 备注 |
|----------------|-------------|-------|
...
| 平台 | OpenClaw 配置路径 | Oria `.env` 变量 | 备注 |
|----------|---------------------|----------------------|-------|
...
| 内容 | 归档文件 | 在 Oria 中的重建方式 |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | 合并至 `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Oria 内置工具说明 |
| `HEARTBEAT.md` | `archive/workspace/HEARTBEAT.md` | 使用 cron 作业执行周期性任务 |
| `BOOTSTRAP.md` | `archive/workspace/BOOTSTRAP.md` | 使用上下文文件或 skills |
| Cron 作业 | `archive/cron-config.json` | 通过 `oria cron create` 重建 |
| 插件 | `archive/plugins-config.json` | 参见 [插件指南](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | 使用 `oria webhook` 或 gateway hooks |
| 记忆后端 | `archive/memory-backend-config.json` | 通过 `oria honcho` 配置 |
| Skills 注册表 | `archive/skills-registry-config.json` | 使用 `oria skills config` |
| UI/身份 | `archive/ui-identity-config.json` | 使用 `/skin` 命令 |
| 日志 | `archive/logging-diagnostics-config.json` | 在 `config.yaml` 日志部分设置 |
| 多 Agent 列表 | `archive/agents-list.json` | 使用 Oria profiles |
...
迁移会解析所有三种格式。对于环境变量模板和 `source: "env"` 的 SecretRef 对象，会从 `~/.openclaw/.env` 和 `openclaw.json` 的 env 子对象中查找值。`source: "file"` 或 `source: "exec"` 的 SecretRef 对象无法自动解析——迁移会对此发出警告，相关值需通过 `oria config set` 手动添加至 Oria。
...
4. **验证 API 密钥** — 运行 `oria status` 检查 provider 认证状态。

5. **测试消息平台** — 若迁移了平台 token，重启 gateway：`systemctl --user restart hermes-gateway`

6. **检查会话归档** — 查看已归档的高级设置；空闲和每日重置计时器不会导入。

7. **重新配对 WhatsApp** — WhatsApp 使用二维码配对（Baileys），不支持 token 迁移。运行 `oria whatsapp` 进行配对。

8. **清理归档** — 确认一切正常后，运行 `oria claw cleanup` 将残留的 OpenClaw 目录重命名为 `.pre-migration/`（防止状态混淆）。
...
根据 OpenClaw 版本不同，密钥可能存储在多个位置：`openclaw.json` 中 `models.providers.*.apiKey` 内联、`~/.openclaw/.env`、`openclaw.json` 的 `"env"` 子对象，或 `agents/main/agent/auth-profiles.json`。迁移会检查所有四个位置。若密钥使用 `source: "file"` 或 `source: "exec"` 的 SecretRef，则无法自动解析——请通过 `oria config set` 手动添加。
...
OpenClaw 在两处存储 TTS 设置：`messages.tts.providers.*` 和顶层 `talk` 配置。迁移会检查两处。若你的 voice ID 是通过 OpenClaw UI 设置的（存储路径不同），可能需要手动设置：`oria config set tts.elevenlabs.voice_id YOUR_VOICE_ID`。