...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Oria already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Oria provider id (e.g. "openrouter")
...
using Oria built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Oria provider
        # (must be one Oria already knows about — same names as
        # `oria model` and config.yaml model.provider).
        ...
        # plugin sends — Oria does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Oria's usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Oria subsystem:
