...
description: "How to build an image-generation backend plugin for Oria"
---
...
Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `oria tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `oria tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.hermes/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `oria plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `oria tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `oria tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `oria tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `oria plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `oria tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `oria tools` model picker |
...
oria plugins enable my-backend
...
# Exercise it
oria -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `oria tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
