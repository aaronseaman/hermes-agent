...
description: "Wire contract between the Oria gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **Oria gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `oria gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`oria gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `oria` CLI.
