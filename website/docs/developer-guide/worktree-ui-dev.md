...
Neither is read by Oria itself — they're private to these helpers. The variables Oria *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `oria --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_hermes_root)" || { echo "htui: not in an Oria checkout" >&2; return 1; }
...
`HERMES_TUI_DIR` points Oria at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `oria --tui --dev` exits with an error. Run `unset HERMES_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_hermes_root)" || { echo "hgui: not in an Oria checkout" >&2; return 1; }
...
|----------|----------------|
| `HERMES_DESKTOP_HERMES_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `oria`. |
| `HERMES_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `HERMES_DESKTOP_IGNORE_EXISTING` | Ignores any `oria` on `PATH` so it can't shadow the worktree. |
...
```bash
# The enclosing worktree, verified as a real Oria checkout.
_hermes_root() {
  local root
...
- [TUI](../user-guide/tui.md) — `oria --tui --dev` and the `HERMES_TUI_DIR` prebuild path
...
- [Environment Variables](../reference/environment-variables.md) — every `HERMES_*` variable Oria reads
