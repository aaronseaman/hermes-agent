...
has to be thrown away or summarized. Oria has always done this in one batch:
...
Oria folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Oria's normal session flush is
...
With `micro_compact` unset or `false` Oria behaves exactly as it always has:
