...
Plugins can launch and supervise fresh Oria child sessions without importing
...
active agent turn fails closed with `No active Oria parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Oria can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Oria's existing unsafe-tool block remains enforced.
