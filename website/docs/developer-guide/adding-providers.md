...
description: "How to add a new inference provider to Oria — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Oria can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `oria model` menu entries
...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `oria setup`.
:::
...
7. `oria model` menu includes the provider
8. `oria setup` wizard delegates to `main.py` automatically
...
- Provider-specific `oria model` menu entries with bespoke auth flows
...
- What env vars should Oria check, and in what priority order?
...
Be careful with API-key precedence. Oria already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hermes_cli/main.py`

A provider is not discoverable until it shows up in the interactive `oria model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `oria model` and `oria setup` automatically.
:::
...
When you add a native provider, double-check that Oria is only sending fields that provider actually understands.
...
### 7. Updating `oria model` but not `oria setup`
