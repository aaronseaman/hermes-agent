...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Oria — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Oria! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Oria core? Start with [Build an Oria Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Oria itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Oria instance (throwaway HERMES_HOME, separate Electron
...
Projects built on (or reviewed by) Oria can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Oria to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
