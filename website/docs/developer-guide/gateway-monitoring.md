...
Oria gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Oria's native NeMo Relay SDK
...
a plain `oria gateway run`: the exporter lives in the gateway process, so
...
Oria Agent-owned Relay transport health remains in scope. A separate gateway
...
```bash
oria monitoring status                   # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `oria monitoring` CLI intentionally exposes `status` only. This first
release covers only Oria Agent-owned service-health and operational-diagnostic
signals, including Oria Agent-owned Relay transport health. Team Gateway's
