...
Oria observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Oria's planner, model providers, memory, tool registry,
...
Oria also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Oria catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Oria only builds
...
The native NeMo Relay SDK integration maps Oria session, turn, LLM, and tool
