...
description: "All three ways to connect Oria to Buzz — Block's Nostr-based human+agent workspace"
---
...
Oria integrates with Buzz three ways. Pick by where Oria runs and what you want it to do:
...
| **Setup** | Automatic discovery | `buzz-acp` env vars | `oria gateway setup` → Buzz |

## ① Buzz Desktop managed runtime

Buzz Desktop ships Oria as a preset runtime. With Oria installed the normal way, open **Settings → Runtimes** and Oria appears automatically — discovery resolves the `hermes-acp` launcher on your login-shell PATH, which the installer writes to `~/.local/bin` (and `oria update` self-heals on older installs).
...
For a hosted Oria identity that joins Buzz *channels* while Buzz's own harness owns the transport:

```text
Buzz relay <-- WebSocket --> buzz-acp <-- ACP over stdio --> Oria
```

The spawned Oria uses the same config, credentials, memory, and skills as `oria` on that host. Key minting, channel discovery, owner-only telemetry (`BUZZ_ACP_RELAY_OBSERVER`), and headless-permission guidance: **[ACP Host Integration → Buzz channels (relay bridge)](/user-guide/features/acp#buzz-channels-relay-bridge)**
...
- **You already run Oria as your agent and want Buzz as another channel** → ③. This is the deepest integration and the one that keeps every Oria feature.

①/② and ③ use different identities and transports; run ③ with its own dedicated Nostr keypair. The adapter takes a scoped lock on the relay+pubkey pair, so two Oria profiles cannot accidentally drive one Buzz identity.
