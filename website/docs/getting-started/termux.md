...
description: "Run Oria directly on an Android phone with Termux"
---

# Oria on Android with Termux
...
Oria can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Oria from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Oria installs are marked with install method `apt`. Oria therefore does not run its Git self-updater against package-owned files; use the package manager instead:
...
Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Oria runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Oria now ships a Termux-aware installer path:
...
- links `oria` into `$PREFIX/bin` so it stays on your Termux PATH
...
Oria requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `oria` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/hermes" "$PREFIX/bin/hermes"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `oria` command persist across new shells without re-activating the venv every time.
...
### `oria doctor` says ripgrep or Node is missing
