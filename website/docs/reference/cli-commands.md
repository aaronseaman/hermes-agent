...
description: "Authoritative reference for Oria terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which Oria profile to use for this invocation. Overrides the sticky default set by `oria profile use`. |
...
| Command | Purpose |
|---------|---------|
| `oria chat` | Interactive or one-shot chat with the agent. |
| `oria model` | Interactively choose the default provider and model. |
| `oria moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `oria fallback` | Manage fallback providers tried when the primary model errors. |
| `oria gateway` | Run or manage the messaging gateway service. |
| `oria proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `oria egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
| `oria lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `oria setup` | Interactive setup wizard for all or part of the configuration. |
| `oria whatsapp` | Configure and pair the WhatsApp bridge. |
| `oria whatsapp-cloud` | Configure the official Meta WhatsApp Business Cloud API adapter (Business account + public webhook required). Distinct from `oria whatsapp` (Baileys personal-account bridge). |
| `oria slack` | Slack helpers (currently: generate the app manifest with every command as a native slash). |
| `oria auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/Nous/Anthropic. |
| `oria login` / `logout` | **Deprecated** — use `oria auth` instead. |
| `oria send` | Send a one-shot message to a configured messaging platform (Telegram, Discord, Slack, Signal, SMS, …). Useful from shell scripts, cron jobs, CI hooks, and monitoring daemons — no agent loop, no LLM. |
| `oria peer` | Register peer Oria gateways on other machines and DM their agents' canonical Bot Chats (`oria peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
| `oria secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.hermes/.env`. |
| `oria migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `oria status` | Show agent, auth, and platform status. |
| `oria cron` | Inspect and tick the cron scheduler. |
| `oria pause` / `oria resume` | Global emergency stop: no new cron fires (built-in ticker, managed-cron webhook, misfire catch-up), kanban dispatch or gateway turns start until resumed; in-flight work is never killed. |
| `oria kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `oria project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `oria webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `oria hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `oria doctor` | Diagnose config and dependency issues. |
| `oria security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `oria approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
| `oria dump` | Copy-pasteable setup summary for support/debugging. |
| `oria prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `oria debug` | Debug tools — upload logs and system info for support. |
| `oria backup` | Back up Oria home directory to a zip file. |
| `oria checkpoints` | Inspect / prune / clear `~/.hermes/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `oria import` | Restore an Oria backup from a zip file. |
| `oria logs` | View, tail, and filter agent/gateway/error log files. |
| `oria config` | Show, edit, migrate, and query configuration files. |
| `oria skin` | List, switch, and tweak display skins. |
| `oria console` | Open the safe Oria command console. |
| `oria pairing` | Approve or revoke messaging pairing codes. |
| `oria skills` | Browse, install, publish, audit, and configure skills. |
| `oria bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `oria curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `oria journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
| `oria memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `oria honcho`) register automatically when their provider is active. |
| `oria acp` | Run Oria as an ACP server for editor integration. |
| `oria mcp` | Manage MCP server configurations and run Oria as an MCP server. |
| `oria plugins` | Manage Oria plugins (install, enable, disable, remove). |
| `oria portal` | Nous Portal status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
| `oria tools` | Configure enabled tools per platform. |
| `oria computer-use` | Install or check the Computer Use (cua-driver) backend (macOS/Windows/Linux). |
| `oria pets` | Browse, install, and select [petdex](../user-guide/features/pets.md) animated pets shown across the CLI, TUI, and desktop app. Subcommands: `list`, `install`, `select`, `show`, `off`, `scale`, `remove`, `doctor`. |
| `oria sessions` | Browse, export, prune, rename, and delete sessions. |
| `oria insights` | Show token/cost/activity analytics. |
| `oria claw` | OpenClaw migration helpers. |
| `oria import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `oria dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `oria serve` | Start the Oria backend server (headless; powers the desktop app and remote backends). |
| `oria desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `oria profile` | Manage profiles — multiple isolated Oria instances. |
| `oria completion` | Print shell completion scripts (bash/zsh/fish). |
| `oria --version` | Show version information. |
| `oria update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `HERMES_HOME` snapshot. |
| `oria uninstall` | Remove Oria from the system. |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Oria itself. |
...
```bash
oria
oria chat -q "Summarize the latest PRs"            # seeds an interactive session
oria chat --oneshot -q "Summarize the latest PRs"    # answer and exit
oria chat --provider openrouter --model anthropic/claude-sonnet-4.6
oria chat --toolsets web,terminal,skills
oria chat --quiet -q "Return only JSON"
oria chat -q "Inspect this repository" --format stream-json
oria chat --worktree -q "Review this repo and open a PR"
oria chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
oria chat --safe-mode -q "Is this bug mine or Oria'?"
```
...
```bash
oria chat -q "Summarize this repository" --format stream-json
```
...
### `oria -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `oria -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
oria -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(oria -z "summarize this" < /path/to/file.txt)
```
...
```bash
oria -z "…" --provider openrouter --model openai/gpt-5.5
# or:
HERMES_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 oria -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `oria chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".

#### `--usage-file` — JSON usage report for pipelines

`oria -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, and `completed` / `failed` flags. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
oria -z "summarize this repo" --usage-file /tmp/usage.json
jq .estimated_cost_usd /tmp/usage.json
```

## `oria model`

Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Oria chat session.
...
:::warning oria model vs /model — know the difference
**`oria model`** (run from your terminal, outside any Oria session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Oria chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Oria session first (`Ctrl+C` or `/quit`), then run `oria model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `oria model` from the terminal.
:::
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Oria Relay](/user-guide/messaging/relay). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `HERMES_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `oria update`. |
...
`oria gateway restart`, or service-restart update exits with status `75`
...
`oria gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `oria gateway run` instead of `oria gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s hermes 'oria gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-oria-gateway-start-fails) for details.
:::
...
```bash
oria setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure] [--portal]
```

**Easiest path:** `oria setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--reconfigure` | Backwards-compat alias — bare `oria setup` on an existing install now does this by default. |
...
For configuration of the gateway itself, see [Tool Gateway](../user-guide/features/tool-gateway.md). For the one-shot setup path, see `oria setup --portal` above.
...
```bash
oria slack manifest                # print manifest to stdout
oria slack manifest --write        # write to ~/.hermes/slack-manifest.json
oria slack manifest --long-description-file AGENTS.md --write
oria slack manifest --slashes-only    # just the features.slash_commands array
```
...
| `--name NAME` | `Oria` | Bot display name in Slack. |
...
Run `oria slack manifest --write` again after `oria update` to pick
up any new commands.
...
```bash
oria send --to <target> "message text"
oria send --to <target> --file <path>
echo "message" | oria send --to <target>
oria send --list [platform]
```
...
For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `oria send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `oria send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
oria send --to telegram "MEDIA:/tmp/screenshot.png"
oria send --to telegram "Build chart for today MEDIA:/tmp/chart.png"     # with caption
oria send --to discord:#ops "MEDIA:/tmp/report.pdf"
```
...
```bash
oria send --to telegram "[[as_document]] MEDIA:/tmp/screenshot.png"
```

Examples:

```bash
oria send --to telegram "deploy finished"
echo "RAM 92%" | oria send --to telegram:-1001234567890
oria send --to discord:#ops --file /tmp/report.md
oria send --to slack:#eng --subject "[CI]" --file build.log
oria send --list                    # all platforms
oria send --list telegram           # filter by platform
```
...
```bash
oria peer add <name> --url http://host:port --key <API_SERVER_KEY>
oria peer list
oria peer dm <peer>[/<agent>] "message"
oria peer run <peer>[/<agent>] --idempotency-key <key> "message"
oria peer status <peer>[/<agent>] <run_id>
oria peer stop <peer>[/<agent>] <run_id>
oria peer remove <name>
```

Bot-to-bot DMs across machines. Register another Oria gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`oria peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`oria -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
includes the peer roster and the `oria peer dm` pattern, so agents discover
...
```bash
oria secrets bitwarden <subcommand>
oria secrets bw <subcommand>            # short alias
```
...
> Not to be confused with `oria claw migrate` (one-shot import of OpenClaw configuration into Oria) — `oria migrate` is the top-level config-rewrite command.


## `oria proxy`

```bash
oria proxy <subcommand>
```
...
```bash
oria security <subcommand>
```

On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Oria venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.hermes/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Oria Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `oria login` / `oria logout` *(Deprecated)*

:::caution
`oria login` has been removed. Use `oria auth` to manage OAuth credentials, `oria model` to select a provider, or `oria setup` for full interactive setup.
:::
...
```bash
oria auth                                                # Interactive wizard
oria auth list                                           # Show all pools
oria auth list openrouter                                # Show specific provider
oria auth add openrouter --api-key sk-or-v1-xxx          # Add API key
oria auth add openrouter --type oauth                    # Browser login (OpenRouter PKCE) mints a key for you
oria auth add anthropic --type oauth                     # Add OAuth credential
oria auth add openai-codex --type oauth --priority 0     # Add an account and try it first
oria auth remove openrouter 2                            # Remove by index
oria auth priority openrouter backup-key 0               # Move a credential to the front of fill_first order
oria auth reset openrouter                               # Clear cooldowns
oria auth reset openrouter 2                             # Clear the cooldown on one credential
oria auth refresh openai-codex work                      # Refresh one OAuth credential and clear its cooldown
oria auth status anthropic                               # Show auth status for a provider
oria auth logout anthropic                               # Log out and clear stored auth state
oria auth spotify                                      # Authenticate Oria with Spotify via PKCE
```
...
```bash
oria cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
```bash
oria kanban [--board <slug>] <action> [options]
```
...
oria kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
oria kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
oria kanban boards switch atm10-server
oria kanban list                    # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
oria kanban boards rm atm10-server
oria kanban boards rm atm10-server --delete
```
...
```bash
oria egress install                    # download the pinned iron-proxy binary
oria egress install --force            # re-download even if already installed

oria egress setup                      # interactive wizard: CA, mappings, config
oria egress setup --tunnel-port N      # override the tunnel listener port (default 9090)
oria egress setup --from-bitwarden     # use Bitwarden Secrets Manager as credential source
oria egress setup --no-bitwarden       # explicitly switch back to env-based credentials
oria egress setup --rotate-tokens      # mint fresh proxy tokens (default preserves existing)

oria egress start                      # spawn the managed proxy daemon
oria egress stop                       # SIGTERM (then SIGKILL after 5s grace)
oria egress restart                    # stop (if running) then start — needed for secret changes
oria egress reload                     # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

oria egress status                     # binary + config + pid + listening + mappings
oria egress status --show-tokens       # print proxy tokens in full (default: redacted)

oria egress disable                    # flip proxy.enabled = false (does not stop a running proxy)
oria egress config                     # print the path to proxy.yaml for inspection
```
...
oria egress setup && oria egress start
oria config set terminal.backend docker     # if not already

# Switching credential source after the fact
oria egress setup --from-bitwarden         # env → bitwarden
oria egress setup --no-bitwarden           # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
oria egress setup --rotate-tokens      # setup offers to restart the running daemon for you
...
oria egress setup
oria egress restart                    # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
oria egress status                       # current state in one view
...
```bash
oria project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
```bash
oria webhook <subscribe|list|remove|test>
```
...
### `oria webhook subscribe`

```bash
oria webhook subscribe <name> [options]
```
...
Outputs a compact, plain-text summary of your entire Oria setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Oria version, release date, git commit hash |
...
:::tip
`oria dump` is specifically designed for sharing. For interactive diagnostics, use `oria doctor`. For a visual overview, use `oria status`.
:::

## `oria debug`

```bash
oria debug share [options]
```
...
The report includes system info (OS, Python version, Oria version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
```bash
oria debug share                # Upload debug report, print URL
oria debug share --lines 500    # Include more log lines
oria debug share --expire 30    # Keep paste for 30 days
oria debug share --nous         # Upload a private diagnostics bundle for Nous support
oria debug share --local        # Print report to terminal (no upload)
```
...
Create a zip archive of your Oria configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Oria is running (WAL-mode safe).
...
```bash
oria backup                             # Full backup to ~/hermes-backup-*.zip
oria backup -o /tmp/hermes.zip          # Full backup to specific path
oria backup --quick                     # Quick state-only snapshot
oria backup --quick --label "pre-upgrade"    # Quick snapshot with label
```

## `oria checkpoints`

```bash
oria checkpoints [COMMAND]
```
...
| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `oria checkpoints` is equivalent. |
...
```bash
oria checkpoints                                    # status overview
oria checkpoints prune --retention-days 3           # aggressive cleanup
oria checkpoints prune --max-size-mb 200            # tighten size cap once
oria checkpoints clear-legacy -f                    # drop v1 archive dirs
oria checkpoints clear -f                           # wipe everything
```
...
```bash
oria import <zipfile> [options]
```

Restore a previously created Oria backup into your Oria home directory. All files in the archive overwrite existing files in your Oria home; `--force` only skips the confirmation prompt that fires when the target already has an Oria installation.
...
### Examples
```bash
oria import ~/hermes-backup-20260423.zip             # Prompts before overwriting existing config
oria import ~/hermes-backup-20260423.zip --force     # Overwrite without prompting
```

## `oria logs`

```bash
oria logs [log_name] [options]
```

View, tail, and filter Oria log files. All logs are stored in `~/.hermes/logs/` (or `<profile>/logs/` for non-default profiles).
...
oria logs gateway -n 100

# Show only warnings and errors from the last hour
oria logs --level WARNING --since 1h

# Filter by a specific session
oria logs --session abc123

# Follow errors.log, starting from 30 minutes ago
oria logs errors --since 30m -f
...
oria logs --level WARNING --since 2h --session tg-12345
```
...
Oria uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `oria logs list` subcommand shows all log files including rotated ones.


## `oria prompt-size`

```bash
oria prompt-size [--platform <name>] [--json]
```
...
- **Prompt tiers** — stable / context / volatile, matching how Oria layers
...
oria prompt-size --platform telegram

# Machine-readable output for scripts
oria prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`oria tools`) or
uninstall skills you don't need (`oria skills`). Context files (AGENTS.md,
...
`oria config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `oria config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `oria config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
```bash
oria pairing <list|approve|revoke|clear-pending>
```
...
```bash
oria skills browse
oria skills browse --source official
oria skills search react --source skills-sh
oria skills search https://mintlify.com/docs --source well-known
oria skills inspect official/security/1password
oria skills inspect skills-sh/vercel-labs/json-render/json-render-react
oria skills install official/migration/openclaw-migration
oria skills install skills-sh/anthropics/skills/pdf --force
oria skills install https://sharethis.chat/SKILL.md                       # Direct URL (+ referenced support files)
oria skills install https://example.com/SKILL.md --name my-skill          # Override name when frontmatter has none
oria skills check
oria skills update
oria skills config
oria skills reset google-workspace
oria skills reset google-workspace --restore --yes
oria skills opt-out                    # stop future bundled-skill seeding (nothing deleted)
oria skills opt-out --remove --yes     # also delete UNMODIFIED bundled skills
oria skills opt-in --sync              # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point Oria at a site exposing `/.well-known/skills/index.json`.
...
```bash
oria bundles create backend-dev \
...
oria bundles list
oria bundles show backend-dev
oria bundles delete backend-dev
```
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `oria update`. Use `oria curator run --dry-run` to preview before that happens.
...
| `add` | Pick a provider + model (same picker as `oria model`) and append to the chain |
...
When an external memory provider is active, it may register its own top-level `hermes <provider>` command for provider-specific management (e.g. `oria honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `oria --help` to see what's currently wired in.
:::
...
Starts Oria as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run Oria as an MCP server.
...
| `install <name>` | Install a catalog entry (e.g. `oria mcp install n8n`). |
| `serve [-v\|--verbose]` | Run Oria as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Oria](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-oria-as-an-mcp-server).

## `oria plugins`

```bash
oria plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `oria plugins` with no subcommand opens a composite interactive screen with two sections:
...
| `install <identifier> [--force] [--ref COMMIT_SHA] [--allow-removed]` | Install a plugin from the Oria plugin catalog (bare entry name), a Git URL, or `owner/repo` shorthand. Catalog names resolve to the entry's repo at its pinned 40-hex commit SHA, show the declared capability summary, and record catalog provenance in a `.hermes-catalog.json` sidecar. Raw URLs are flagged as custom (unreviewed) sources; `--ref` (full 40-character commit SHA) pins them. `--allow-removed` (DANGEROUS) bypasses the removed-plugin blocklist. |
| `search [term] [--json]` | Search the Oria plugin catalog (matches entry names, descriptions, and declared tools; omit `term` to list everything). The catalog is curated in-repo (`plugin-catalog/`), refreshed from the live repo with a 6-hour cache, and falls back to the in-tree copy offline. Cataloged ≠ audited — admission reviews the entry, not the code. |
...
See [Plugins](../user-guide/features/plugins.md) and [Build an Oria Plugin](../developer-guide/plugins/index.md).
...
`oria computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`oria tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, Oria checks its version and runtime
...
installer. Oria never replaces a custom binary selected through
`HERMES_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`oria computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended Oria integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Oria and links Cua's
skill pack into the Oria skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode Oria passes Cua's canonical
...
`oria update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Oria update.

## `oria pets`

```bash
oria pets <list|install|select|show|off|scale|remove|doctor>
```

[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Oria shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
```bash
oria insights [--days N] [--source platform]
```
...
```bash
oria claw migrate [options]
```

Migrate your OpenClaw setup to Oria. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Oria files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hermes/` (by default a single restore-point archive is written to `~/.hermes/backups/pre-migration-*.zip` before apply; restorable with `oria import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Oria equivalents or **archived** for manual review.
...
oria claw migrate --dry-run

# Full migration (all compatible settings, no secrets)
oria claw migrate --preset full

# Full migration including API keys
oria claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
oria claw migrate --preset user-data --overwrite

# Migrate from a custom OpenClaw path
oria claw migrate --source /home/user/old-openclaw
```

## `oria import-agent`

```bash
oria import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Oria. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.hermes/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Start the Oria **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](/user-guide/desktop) and remote clients connect to. It is the same server `oria dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `oria serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `oria dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.

**Port conflicts:** if the requested port (default `9119`) is already held by another process (e.g. a second `oria serve` or the gateway), the command prints a machine-readable sentinel line `BACKEND_PORT_IN_USE port=<port>` to stdout, a human hint naming the likely holder, and exits with code **75** (`EX_TEMPFAIL`) instead of a generic error — so scripts and the desktop app can tell "port occupied" apart from "backend broken". Pass `--port 0` to bind a free ephemeral port (the successful boot announces the chosen port via `HERMES_BACKEND_READY port=<port>`).

## `oria dashboard`

```bash
oria dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`oria serve`](#oria-serve) above.) Requires `cd ~/.hermes/hermes-agent && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.hermes/hermes-agent && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
| `--stop` | — | Stop running `oria dashboard` processes and exit. |
| `--status` | — | List running `oria dashboard` processes and exit. |

### `oria dashboard register`

Register this install as a self-hosted dashboard with your Nous Portal account. Creates an OAuth client, writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hermes/.env`, and prints how to engage the login gate. Requires being logged in (`oria setup`).
...
oria dashboard --port 8080 --no-open
...
```bash
oria profile <subcommand>
```

Manage profiles — multiple isolated Oria instances, each with its own config, sessions, skills, and home directory.
...
```bash
oria profile list
oria profile create work --clone
oria profile create work --clone --sync-imports     # also carry over the import-agent sync manifest
oria profile use work
oria profile alias work --name h-work
oria profile export work -o work-backup.tar.gz
oria profile import work-backup.tar.gz --name restored
oria profile install github.com/user/my-distro --alias
oria profile update work
oria -p work chat -q "Hello from work profile"
```

## `oria completion`

```bash
oria completion [bash|zsh|fish]
```

Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Oria commands, subcommands, and profile names.

Examples:

```bash
# Bash
oria completion bash >> ~/.bashrc

# Zsh
oria completion zsh >> ~/.zshrc

# Fish
oria completion fish > ~/.config/fish/completions/hermes.fish
```

## `oria update`

```bash
oria update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```
...
`oria update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Oria may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `oria config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Oria attempts to restart all running gateway profiles automatically so they pick up the new code. Use `oria gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, Oria makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `oria update`.
- **Pairing data snapshot.** Even when `--backup` is off, `oria update` takes a lightweight snapshot of `~/.hermes/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `oria backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `hermes.service` warning.** If Oria detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `oria --version` | Print version information. |
| `oria update` | Pull latest changes and reinstall dependencies. |

| `oria uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Oria, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
