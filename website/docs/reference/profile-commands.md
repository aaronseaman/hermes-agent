...
This page covers all commands related to [Oria profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `oria profile` without a subcommand shows help.
...
```bash
oria profile use <name>
```

Sets `<name>` as the active profile. All subsequent `oria` commands (without `-p`) will use this profile.
...
oria profile use default
```

## `oria profile create`

```bash
oria profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `oria profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `oria update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.hermes`), use `oria skills opt-out` / `oria skills opt-in`. |
...
oria profile create work2 --clone-from work

# Clone everything from a specific profile
oria profile create work2-backup --clone-from work --clone-all
```

## `oria profile describe`

```bash
oria profile describe [<name>] [options]
```
...
oria profile describe researcher

# Set it explicitly
oria profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
oria profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
oria profile describe --all --auto
```

## `oria profile delete`

```bash
oria profile delete <name> [options]
```
...
```bash
oria profile delete mybot
oria profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.hermes`) cannot be deleted — use `oria uninstall` to remove everything.
:::

## `oria profile show`

```bash
oria profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Oria home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
oria profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Oria installation.
...
```bash
oria profile alias work
# Creates/updates ~/.local/bin/work

oria profile alias work --name mywork
# Creates ~/.local/bin/mywork

oria profile alias work --remove
...
```bash
oria profile rename <old-name> <new-name>
```
...
```bash
oria profile rename mybot assistant
...
```bash
oria profile export <name> [options]
```
...
```bash
oria profile export work
# Creates work.tar.gz in the current directory

oria profile export work -o ./work-2026-03-29.tar.gz
```
...
```bash
oria profile import <archive> [options]
```
...
```bash
oria profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

oria profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `oria -p telemetry`). |
...
oria profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
oria profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
oria profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
oria profile install ./telemetry/
```

### `oria profile update`

```bash
oria profile update <name> [--force-config] [--yes]
```
...
```bash
oria profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Oria version, author, env var requirements, the source URL/path, and
...
`oria profile list` also shows the distribution name and version in a
`Distribution` column, and `oria profile show <name>` / `delete <name>`
...
oria profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
oria profile install https://github.com/your-org/internal-assistant.git
```
...
Oria version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Oria can clone from.
3. Tell recipients to run `oria profile install <your-repo-url>`.
...
```bash
oria -p <name> <command> [options]
oria --profile <name> <command> [options]
```

Global flag to run any Oria command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
oria -p work chat -q "Check the server status"
oria --profile dev gateway start
oria -p personal skills list
oria -p work config edit
```

## `oria completion`

```bash
oria completion <shell>
```
...
- `oria profile <TAB>` — subcommands (list, use, create, etc.)
- `oria profile use <TAB>` — profile names
- `oria -p <TAB>` — profile names
