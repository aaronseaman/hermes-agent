...
description: "Reference for Oria MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Oria may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Oria only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Oria does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Oria uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, client identification, token exchange, and refresh)
...
device authorization from a terminal on the machine running Oria:

```bash
oria mcp login protected_api --flow device
```

Open the printed verification URL on any device and enter the displayed user code.
Oria polls for approval, respects `authorization_pending` and `slow_down`, and stops
...
Set `oauth.flow: device` on the server to make `oria mcp login` and `oria mcp reauth`
...
Oria identifies itself to authorization servers with a **Client ID Metadata Document** (CIMD), the mechanism the MCP `2026-07-28` spec adopted in place of Dynamic Client Registration. The document is published at
`https://nousresearch.github.io/hermes-agent/docs/oauth/client-metadata.json`, and that URL *is* the `client_id` — the authorization server fetches it to learn Oria's name, logo, and permitted redirect URIs. Nothing is registered per install, and nothing is user-specific.
...
The document declares a fixed set of loopback redirect URIs, and the spec requires the redirect URI in an authorization request to be an *exact string match* against one of them — so a CIMD flow cannot use the random high port Oria normally picks. Oria therefore pins the callback to one of ports `27890`–`27894`.

That pin has to be chosen before the server's capabilities are known, because the redirect URI is fixed at the start of the flow while the server's metadata only arrives partway through. So Oria pins the port for any flow that *could* end up using CIMD, and reverts to a random port for the rest:

- A server Oria has connected to before, whose cached metadata does not advertise CIMD, keeps the random port it has always used.
- A server Oria has never reached gets a pinned port on that first login, since guessing is the only way CIMD can ever be used.
...
If a server fetches the document and refuses it at the *token* endpoint (`invalid_client`), Oria logs the rejection, records it under `~/.hermes/mcp-tokens/<server>.cimd-off`, and uses DCR for that server from then on.

A server that cannot fetch or validate the document at all aborts at the *authorization* endpoint instead, before any redirect happens. There is no signal Oria can observe there, so the browser shows an invalid-client error and the login times out after five minutes. The timeout message names the document and points at `cimd: false`. Running `oria mcp login <server>` clears the recorded rejection, so a corrected document gets another chance.
...
MCP vendors and docs can offer a one-click **"Add to Oria"** button that opens the Oria desktop app with a pre-filled server config, mirroring Cursor's `cursor://anysphere.cursor-deeplink/mcp/install` scheme:
