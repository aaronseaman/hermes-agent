...
description: "Frequently asked questions and solutions to common issues with Oria"
---
...
### What LLM providers work with Oria?

Oria works with any OpenAI-compatible API. Supported providers include:
...
- **Anthropic** — Claude models (direct API, OAuth via `oria auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `oria model` or by editing `~/.hermes/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Oria in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Oria inside WSL2
...
- let Oria use the resulting MCP browser tools

This is more reliable than trying to force Oria core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Oria](../guides/use-mcp-with-hermes.md#wsl2-bridge-oria-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Oria does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.hermes/`.

### Can I use it offline / with local models?

Yes. Run `oria model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Oria minimum; set this to match your server's actual context window
```
...
Oria persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Oria — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Oria auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Oria itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Oria instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Oria programmatically:
...
**Cause:** Oria requires Python 3.11 or newer.
...
**Cause:** Oria builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Oria runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Oria auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.hermes/config.yaml`:
...
Independently of the init files, every terminal command's `PATH` is completed with the standard system directories (`/usr/local/bin`, `/opt/homebrew/bin`, …), the Oria-managed runtime dirs, and `~/.local/bin` when it exists (the `pip --user` / `pipx` / `uv tool` install target) — appended after your own entries, so precedence is unchanged. This covers backends started with a thin non-interactive PATH (systemd, GUI launchers, the Desktop SSH remote backend) without any configuration.
...
#### The agent says "Oria policy" or "Oria guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Oria runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Oria enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Oria does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Oria tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not an Oria execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](/user-guide/security) for Oria's documented execution controls and [Providers](/integrations/providers) for provider configuration.
...
**Solution:** Exit your session and use `oria model` from your terminal to add new providers:

```bash
# Exit the Oria chat session first (Ctrl+C or /quit)
...
After adding a new provider via `oria model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `oria model` (from terminal) |
| Enter/change API keys | `oria model` (from terminal) |
...
# Or set directly
oria config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```
...
# Set a valid model
oria config set model.default anthropic/claude-opus-4.7

# Or specify per-session
oria chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `oria chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Oria detected the wrong context length for your model.
...
oria chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Oria may have the wrong context length for your model. Check what it detected:
...
**Cause:** Oria detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Oria never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `oria chat`
...
**Solution:**
- Verify your bot token is valid with `oria gateway setup`
...
#### WSL: Gateway keeps disconnecting or `oria gateway start` fails
...
tmux new -s hermes 'oria gateway run'
# Reattach later: tmux attach -t hermes

# Option 3: Background via nohup
nohup oria gateway run > ~/.hermes/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'oria gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `oria gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
oria gateway install      # Re-snapshots your current PATH
oria gateway start        # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `oria chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `oria chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`oria prompt-size`](/reference/cli-commands#oria-prompt-size) to measure it, then trim: disable toolsets you don't use (`oria tools`) and uninstall or disable skills you don't need (`oria skills`).
...
oria config show | grep -A 12 mcp_servers

# Restart Oria or reload MCP after config changes
oria chat
```
...
:::warning
If an MCP server crashes mid-request, Oria will report a timeout. Check the server's own logs (not just Oria logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `oria profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Oria home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `oria update`?

`oria update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `oria update` once — it covers every profile on the machine.
...
**Solution: Delegation config.** Oria can route subagents to a different model automatically. Set this in `~/.hermes/config.yaml`:
...
Now when you tell Oria "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Oria do this?

**Current limitation:** Oria profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Oria reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `oria skills config` settings don't seem to take effect.

**Solution:** Use `oria skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`oria gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Oria creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Oria to another machine
...
1. Install Oria on the new machine:
   ```bash
   curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
   ```
...
3. Copy the zip to the new machine and import it:
   ...
   # On the new machine
   oria import ~/hermes-backup-<timestamp>.zip
   ```

4. On the new machine, run `oria setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
oria profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
oria profile import ./work-backup.tar.gz work
```
...
### `oria backup` vs `oria profile export`

| Feature | `oria backup` | `oria profile export` |
| :--- | :--- | :--- |
...
:::tip
`oria backup` produces a consistent snapshot even while Oria is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Oria installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not an Oria-specific issue — it's a shell config permissions problem.
...
oria config show | head -20

# Re-run model selection
oria model

# Or test with a known-good model
oria chat -q "hello" --model anthropic/claude-opus-4.7
```
...
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Oria version (`oria --version`), and the full error message
