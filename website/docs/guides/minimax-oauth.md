...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Oria — no API key required"
---

# MiniMax OAuth

Oria supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Oria automatically refreshes your session.
...
# → Oria opens your browser to the MiniMax authorization page
...
```bash
oria auth add minimax-oauth --no-browser
```

Oria will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Oria implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Oria generates a PKCE verifier / challenge pair and a random state value.
...
4. Oria polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
...
```bash
oria config set model.default MiniMax-M2.7
oria config set model.provider minimax-oauth
```
...
```bash
oria --provider minimax-oauth      # canonical
oria --provider minimax-portal     # alias
oria --provider minimax-global     # alias
oria --provider minimax_oauth      # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `oria setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
oria --provider minimax-oauth
```
...
Oria refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Oria marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Oria marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `oria auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Oria raises a timeout error.

**Fix:** re-run `oria auth add minimax-oauth` (or `oria model`). The flow starts fresh.

### State mismatch (possible CSRF)

Oria detected that the `state` value returned by the authorization server does not match what it sent.
...
If `oria` cannot open a browser window, use `--no-browser`:

```bash
oria auth add minimax-oauth --no-browser
```

Oria prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `oria model` and select MiniMax (OAuth), or run `oria auth add minimax-oauth`.
...
```bash
oria auth logout minimax-oauth
```
...
- [oria doctor](../reference/cli-commands.md)
