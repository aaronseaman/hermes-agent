...
description: "Connect Oria to the Nous Portal MCP server so your local agent can list, start, stop, and manage your Hermes Cloud instances conversationally"
---

# Manage Hermes Cloud with MCP

[Hermes Cloud](https://portal.nousresearch.com/cloud) runs hosted Oria instances for you. Normally you manage them from the `/agents` page in the [Nous Portal](/integrations/nous-portal). This guide connects your **local** Oria to the Portal's MCP server so you can manage those cloud instances by just asking — "list my cloud agents", "restart the stopped one", "what's it costing me" — without leaving your terminal.

It's a standard [MCP](/user-guide/features/mcp) server hosted by Nous Research, gated by the same OAuth login you already use for the Portal. Once connected, Oria gets two tools it can call on your behalf.
...
```bash
oria mcp add --url https://portal.nousresearch.com/mcp --auth oauth hermes-cloud
```

`--auth oauth` tells Oria this is an OAuth-protected HTTP server. On first connect Oria:
...
If you ever need to point the connection at a different org, remove and re-add the server (`oria mcp remove hermes-cloud`, then the `add` command again) and pick the other org in the browser.

## Step 2: verify it connected

```bash
oria mcp test hermes-cloud
```
...
Oria reports what each tool returned — the instance list, the new status, the created instance's details — so you can confirm the action landed.

## Configuration

After `oria mcp add`, the server lives in `~/.hermes/config.yaml`:
...
Run `/reload-mcp` after changing the config. See [Use MCP with Oria](/guides/use-mcp-with-hermes) for the full filtering model (`include`/`exclude`, `prompts`, `resources`).
...
If they're still missing, run `oria mcp test hermes-cloud` to see the connection error directly.

### It asks me to log in again

OAuth tokens refresh automatically, but if the Portal invalidates your session (password change, revoke, expiry) the next call asks you to re-authorize. Re-run the `oria mcp add` command — the browser flow re-mints a token.

### Headless / SSH / remote host

The OAuth browser callback runs on the machine where Oria is running. On a remote host, forward the loopback port over SSH — the same pattern as any other OAuth login. See [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh).

## See also

- **[Nous Portal](/integrations/nous-portal)** — the subscription, models, and Tool Gateway behind the same login
- **[Use MCP with Oria](/guides/use-mcp-with-hermes)** — connecting and filtering MCP servers in general
- **[MCP feature overview](/user-guide/features/mcp)** — what MCP is and how Oria uses it
