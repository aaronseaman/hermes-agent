...
Oria calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Oria writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Oria:** *(writes `~/.hermes/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
>
> **Oria:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `oria cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Oria:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
# 3. Verify
oria cron list
oria cron run <job_id>      # fire it once to test
```
...
```bash
oria cron create "every 5m"          # interval
oria cron create "every 2h"
oria cron create "0 9 * * *"         # standard cron: 9am daily
oria cron create "30m"               # one-shot: run once in 30 minutes
```
...
```bash
oria cron list                                      # see all jobs
oria cron pause <job_id>                            # stop firing, keep definition
oria cron resume <job_id>
oria cron edit <job_id> --schedule "every 10m"      # adjust cadence
oria cron edit <job_id> --agent                     # flip to LLM mode
oria cron edit <job_id> --no-agent --script …       # flip back
oria cron remove <job_id>                           # delete it
```
...
For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to an Oria webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Oria being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
