...
Copy-paste blueprints for common automation patterns. Each blueprint uses Oria's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`oria webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `oria webhook subscribe`) |
...
```bash
oria webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
oria webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
oria webhook subscribe alert-triage \
...
```bash
oria cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Oria development." \
...
```bash
oria webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
oria webhook subscribe stripe-payments \
