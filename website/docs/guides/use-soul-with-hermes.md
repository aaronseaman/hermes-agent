---
sidebar_position: 7
title: "Use SOUL.md with Oria"
description: "How to use SOUL.md to shape Oria's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Oria

`SOUL.md` is the **primary identity** for your Oria instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Oria to feel like the same assistant every time you talk to it — or if you want to replace the Oria persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Oria should be
- what Oria should avoid stylistically
- how Oria should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Oria is and how Oria speaks
...
Oria now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run Oria with a custom home directory, it becomes:
...
Oria automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Oria does not overwrite it
- if the file exists but is empty, Oria adds nothing from it to the prompt

## How Oria uses it

When Oria starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Oria feels.
...
Oria already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Oria should sound.

# Avoid
What Oria should not do.

# Defaults
How Oria should behave when ambiguity appears.
```
...
Then restart Oria or start a new session.
...
### I edited SOUL.md but Oria still sounds the same
...
### Oria is ignoring parts of my SOUL.md
