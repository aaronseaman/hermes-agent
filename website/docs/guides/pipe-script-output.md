...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `oria send`."
---

# Pipe Script Output to Messaging Platforms

`oria send` is a small, scriptable CLI that pushes a message to any
messaging platform Oria is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | oria send --to slack:#builds`)
...
Any platform Oria ships adapters for works as a target:
...
`oria send` resolves the message body in this order:

1. **Positional argument** — `oria send --to telegram "hi"`
2. **`--file PATH`** — `oria send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | oria send --to telegram`

When stdin is a TTY (no pipe), Oria does **not** wait for input — you'll
...
Because `oria send` reuses your Oria config, the same script works on
any host where Oria is installed — no need to export bot tokens into
...
`oria send`. If the Python interpreter can't load because the box is
...
## Does `oria send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `oria send` calls
...
pointing at the gateway; start it with `oria gateway start` and retry.
...
"no channels discovered yet", start the gateway once (`oria gateway
...
| Approach | Multi-platform | Reuses Oria creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `oria send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`oria send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `oria send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `oria send` shares with cron delivery.
