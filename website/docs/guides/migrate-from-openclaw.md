...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Oria — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`oria claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Oria. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`oria import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, `oria setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
oria claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hermes/` (by default a single restore-point archive is written before apply, under `~/.hermes/backups/pre-migration-*.zip`; restorable with `oria import`). |
...
| What | OpenClaw source | Oria destination | Notes |
|------|----------------|-------------------|-------|
...
| Source | OpenClaw location | Oria destination |
|--------|------------------|-------------------|
...
Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Oria skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Oria destination | Notes |
|------|---------------------|-------------------|-------|
| Default model | `agents.defaults.model` | `config.yaml` → `model` | Can be a string or `{primary, fallbacks}` object |
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `oria update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
...
| What | OpenClaw config path | Oria config path | Mapping |
|------|---------------------|-------------------|---------|
...
Idle and daily reset timers are not imported: Oria conversations persist until an explicit `/new` or `/reset`. Advanced session settings (identity links, thread bindings, maintenance, scope and send policy) remain archived for reference.

### MCP servers

| OpenClaw field | Oria field | Notes |
|----------------|-------------|-------|
...
| Platform | OpenClaw config path | Oria `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Oria path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Oria equivalent)

These are saved to `~/.hermes/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Oria |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Oria has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `oria cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `oria webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `oria honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `oria skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Oria profiles |
...
4. **Verify API keys** — run `oria status` to check provider authentication.
...
7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `oria whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `oria claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `oria config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
