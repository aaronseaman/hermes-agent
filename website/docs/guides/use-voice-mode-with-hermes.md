---
sidebar_position: 8
title: "Use Voice Mode with Oria"
description: "A practical guide to setting up and using Oria voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Oria
...
- you want Oria sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Oria.
...
## Step 1: make sure normal Oria works first
...
Oria supports both local and cloud speech stacks.
...
### If you use `oria setup`

If you choose NeuTTS in the setup wizard, Oria checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Oria transcribes and responds
...
- asking Oria to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Oria loop.

## Tuning CLI behavior

### Silence threshold

If Oria starts/stops too aggressively, tune:
...
Oria stays a normal chat bot, but can speak replies.
...
- you want Oria to function like a portable research or ops assistant
...
Oria joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Oria detects speech boundaries
- transcripts are posted in the associated text channel
- Oria responds in text and audio
...
1. get text Oria working
2. run `oria setup tts` to enable voice support
