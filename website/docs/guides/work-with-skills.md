...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Oria new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Oria how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Oria installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Oria to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Oria but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
oria skills install official/research/arxiv
...
For how to ship skills in your own plugin, see [Build an Oria Plugin → Bundle skills](/developer-guide/plugins#bundle-skills).
...
When a skill with config is first loaded, Oria prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
oria skills config gif-search

# View all skill config
oria config get skills.config --json
```
...
```bash
oria chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.hermes/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Oria may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Oria will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.
...
**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Oria to update the skill with what you learned. Skills that aren't maintained become liabilities.
