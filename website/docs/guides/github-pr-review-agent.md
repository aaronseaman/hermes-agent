...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Oria          ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Oria instantly when PRs are opened or updated.
:::
...
- **Oria installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  oria gateway install     # Install as a service
  # or
...
Make sure Oria can access GitHub. Start a chat:
...
Still in the chat, ask Oria to review a real PR:
...
A skill gives Oria consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.
...
Verify it loaded — start `oria` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Oria your team's standards:
...
2. Teach Oria your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
oria gateway status      # Is the gateway running?
oria cron list           # Is the job enabled?
```
