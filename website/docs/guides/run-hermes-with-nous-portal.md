...
OAuth needs a browser, but the loopback callback runs on the machine where Oria is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
oria setup --portal                                # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
oria auth add nous --type oauth
# Then re-run `oria setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Oria release notes" and summarize the top 3 hits.
```

You should see Oria call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`oria setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `oria tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Oria runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Oria will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Oria profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.
...
### `oria portal info` shows "not logged in" after `oria setup --portal`
...
```bash
oria config set model.provider nous
```
...
Re-verify with `oria portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Oria doesn't replay it endlessly. Just log in again:
...
`oria portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Oria profiles where you're using the wrong one (check `oria profile list`)

### Want to revoke and start clean

```bash
oria auth logout nous         # wipes the local refresh token
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Oria configurations
