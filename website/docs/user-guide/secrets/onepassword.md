...
3. Every time `oria` (or the gateway, or a cron job) starts, after `~/.hermes/.env` has loaded, Oria runs `op read` for each reference and sets the resolved values into `os.environ`.
4. By default Oria **overrides** values already in your environment, so 1Password is the source of truth — rotate a credential once and every Oria process picks it up on next start. Flip `override_existing: false` if you want `.env` to win instead.

Oria never authenticates on your behalf and never downloads `op`: it shells out to your already-installed, already-trusted CLI. If `op` is missing, your session is locked, or a reference is wrong, Oria prints a one-line warning and continues with whatever credentials `.env` already had — it never blocks startup.

## Authentication

`op` supports two non-interactive-friendly modes; Oria works with either:
...
- **Desktop / interactive sessions** (laptops): run `op signin` (or enable CLI integration in the 1Password app). Oria passes your `OP_SESSION_*` variables through to the `op` child process. The 1Password cache key includes those session variables, so signing into a different account never serves a value cached under the previous identity.

## Bootstrap token

When you authenticate with a **service-account token**, that token is itself the bootstrap credential Oria needs *before* it can resolve any `op://` reference. It must be present in `os.environ` of every process that resolves secrets — including cron jobs (`kanban.dispatch_in_gateway: false`), subprocess invocations, CLI runs, macOS launchd agents, and Docker containers — not just the interactive gateway. There are three ways to make it available, in order of precedence:

1. **In `~/.hermes/.env` (recommended).** `oria secrets onepassword setup --token <token>` writes the token to `~/.hermes/.env`, exactly like Bitwarden's `BWS_ACCESS_TOKEN`. Because `load_hermes_dotenv()` always loads `.env`, the token is available everywhere with zero extra setup. This is the simplest reliable option.
...
2. **In `~/.hermes/.op.env` (gitignored).** If you'd rather keep the service-account token out of `.env` — for example so `.env` can be checked into a private dotfiles repo while the token stays out of version control — place it in `~/.hermes/.op.env`:
   ...
   Oria auto-loads `.op.env` at startup, **after** `.env`, and **never** overrides a token already present in the environment. `.op.env` is gitignored so the token never enters a committed file.
...
3. **Via systemd `EnvironmentFile` (Linux gateway).** If you run the gateway under systemd, you can inject the token directly into the service environment:
   ...
   A token injected this way takes precedence — Oria detects that `OP_SERVICE_ACCOUNT_TOKEN` is already set and skips loading `.op.env` entirely.
...
```bash
oria secrets onepassword setup
```
...
```bash
oria secrets onepassword set OPENAI_API_KEY      "op://Private/OpenAI/api key"
oria secrets onepassword set ANTHROPIC_API_KEY "op://Private/Anthropic/credential"
```

### 4. Preview and confirm

```bash
oria secrets onepassword sync       # dry-run: resolve now, show what would apply
oria secrets onepassword status     # config + binary + references + auth
```

From now on, every `oria` invocation resolves the references at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `oria secrets onepassword setup` | Verify `op`, set account / token env var, enable |
| `oria secrets onepassword status` | Show config, binary, auth, and configured references |
| `oria secrets onepassword token` | Rotate the service-account token: validate with `op whoami`, then store it in `.env` |
| `oria secrets onepassword set ENV_VAR "op://…"` | Map an env var to a reference (stored stripped + validated) |
| `oria secrets onepassword remove ENV_VAR` | Drop a mapping |
| `oria secrets onepassword sync` | Dry-run: resolve references now and show what would apply |
| `oria secrets onepassword sync --apply` | Resolve and export into the current shell's environment |
| `oria secrets onepassword disable` | Flip `enabled: false`; leaves mappings in place |
...
| `service_account_token_env` | `OP_SERVICE_ACCOUNT_TOKEN` | Env var Oria reads the service-account token from. Its value is exported to the `op` child as `OP_SERVICE_ACCOUNT_TOKEN` (the name `op` expects). Leave the var unset to use a desktop/interactive session. |
...
1Password never blocks Oria startup. If anything goes wrong you'll see a one-line warning in stderr and Oria continues:

| Symptom | Cause | Fix |
|---|---|---|
| `the op CLI was not found on PATH` | `op` not installed / not on PATH | Install the CLI, or set `secrets.onepassword.binary_path` |
| `op read failed for 'op://…': …` | Locked session, expired token, or no vault access | `op signin`, run `oria secrets onepassword token` to rotate the service-account token, or grant the service account access |
...
Successful, complete pulls are cached in-process and on disk under `<hermes_home>/cache/op_cache.json` (written atomically, mode `0600`), so back-to-back short-lived `oria` invocations don't re-shell `op` for every reference. The cache:
...
- Oria refuses to let a resolved value overwrite the token env var itself, even with `override_existing: true`.
- The `op` child process gets a minimal allowlisted environment (auth/session vars + `PATH`/`HOME` and the `op` config-location vars `OP_CONFIG_DIR`/`XDG_CONFIG_HOME`), not a copy of the full `os.environ`, so post-dotenv provider credentials aren't all inherited by the child. Set `OP_CONFIG_DIR` when `~/.config` is not writable by the Oria user (common in containers).
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or anywhere you want centralized rotation and revocation across multiple Oria installations.
