...
| | |
|---|---|
| Source | Optional — install with `oria skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents\grok` |
| Version | `0.1.1` |
| Author | Matt Maximo (MattMaximo), Oria |
| License | MIT |
...
# Grok Build CLI — Oria Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Oria terminal. Grok
...
1. Prepare stable input files first with Oria tools (`read_file`,
...
2. **Don't conflate Oria's xAI auth with the `grok` CLI's auth.** Oria'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Oria's xAI auth carries
   over.
