---
title: "Honcho — Configure and troubleshoot Honcho memory for Oria"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Oria"
---
...
Configure and troubleshoot Honcho memory for Oria.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `oria skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Oria

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Oria profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this Oria profile's AI peer
...
Honcho models conversations as interactions between **peers**. Oria creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Oria instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Oria run |
...
Each Oria profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for Oria when Honcho memory is active.
...
| `hermes honcho sync` | Create host blocks for all Oria profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Oria + Honcho |
