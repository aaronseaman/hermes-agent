...
| | |
|---|---|
| Source | Optional — install with `oria skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp\mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Oria |
| License | MIT |
...
# MCP OAuth on a Remote Oria Gateway
...
2. Oria is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Oria** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
3. Starts an HTTP server on `127.0.0.1:P` **inside the Oria process**.
4. Prints the authorize URL and waits for the code at its local endpoint.

When Oria runs remotely, the `127.0.0.1` in the `redirect_uri` is the remote
...
laptop and fails to connect. The callback never reaches the Oria process, the
...
already cover the deployment. When Oria detects a remote session it prints two
...
Both require an interactive terminal to the Oria host. The rest of this skill
is for when there is NO interactive TTY — Oria running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Oria Dashboard (try this BEFORE manual token surgery)

A remote Oria gateway often also runs the **dashboard** web UI as a SEPARATE
...
UA** — Cloudflare will 403 you even though Oria (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Oria sees `auth: oauth`, calls `HermesTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Oria process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Oria needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Oria does natively).
...
- **Don't claim Oria doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
