...
| | |
|---|---|
| Source | Optional — install with `oria skills install official/productivity/telephony` |
...
This optional skill gives Oria practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn Oria into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Oria memory
...
### 1) "I want Oria to own a real phone number"
Use **Twilio**.
...
- pairs well with Oria `text_to_speech` plus a public file host or tunnel
...
Then save credentials into Oria:
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Oria calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Oria's existing `text_to_speech` support.

Use this when:
- you want the call to use Oria's configured TTS voice rather than Twilio `<Say>`
...
Recommended Oria TTS -> Twilio Play workflow:

1. Generate the audio with Oria `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Oria to create the message audio with `text_to_speech`
...
Important note:
- Oria TTS is great for prerecorded outbound messages
...
- Oria STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Oria memory.
...
- Do not store arbitrary third-party phone numbers in Oria memory.
