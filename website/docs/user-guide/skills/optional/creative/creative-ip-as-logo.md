...
| | |
|---|---|
| Source | Optional — install with `oria skills install official/creative/ip-as-logo` |
| Path | `optional-skills/creative/ip-as-logo` |
| Version | `1.0.0` |
| Author | s1dashu (https://github.com/s1dashu, upstream s1dashu/ip-as-logo-skill), ported by Oria |
| License | MIT |
...
> **Oria adaptation notes** (the rest of this document is the upstream
...
8. Determine the available image-generation path before promising output. In Oria this is the `image_generate` tool; if it reports no configured backend, ask the user to enable one (`hermes tools`) instead of fabricating results.
