...
| | |
|---|---|
| Source | Optional — install with `oria skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Oria MCP catalog. The
...
Two halves, in this order: the editor side must be up before Oria connects.
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for Oria — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Oria connects
...
2. Start the Oria session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Oria
they appear as:
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Oria runs batched calls
...
- **Start order matters.** Editor + server up first, then the Oria
