---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Oria"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Oria"
---
...
Set up Actual Computer (actual.inc) inference in Oria.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `oria skills install official/devops/actual-setup` |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as an Oria inference
...
- User has an `ac_` key and wants Oria routed through their Actual cluster.
...
- Oria has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Oria — the
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Oria at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
...
1. **reasoning_effort trap (handled by Oria since the first-class provider).**
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Oria the built-in provider wins the name; stale custom blocks
