---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Oria"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Oria"
---
...
Import an OpenClaw setup (memories, skills) into Oria.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `oria skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Oria (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Oria`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Oria Migration

Use this skill when a user wants to move their OpenClaw setup into Oria with minimal manual cleanup.
...
- archive non-secret docs that do not have a direct Oria destination
...
Oria CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Oria. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into an Oria workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Oria skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Oria unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Oria targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Oria destination. Unsupported auth blobs must still be reported as skipped.
...
- Oria persona state imported
- Oria memory files populated with converted OpenClaw knowledge
