...
| | |
|---|---|
| Source | Optional — install with `oria skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Oria |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Oria terminal calls (desktop app flow only)
...
### Service Account (recommended for Oria)
...
## Oria Execution Pattern (desktop app flow)

Oria terminal commands are non-interactive by default and can lose auth context between calls.
