...
This skill is separate from the Oria Email gateway adapter. The gateway
adapter lets people email the agent and uses Oria's built-in IMAP/SMTP
...
To reply non-interactively from Oria, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Oria)** — pipe the message via stdin:
