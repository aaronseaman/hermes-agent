...
description: "Run Oria natively on Windows 10 / 11 — install, feature matrix, UTF-8 console, Git Bash, gateway as a Scheduled Task, editor handling, PATH, uninstall, and common pitfalls"
...
Oria runs natively on Windows 10 and Windows 11 — no WSL, no Cygwin, no Docker. This page is the deep dive: what works natively, what's WSL-only, what the installer actually does, and the Windows-specific knobs you might need to touch.
...
No admin rights required. The installer goes to `%LOCALAPPDATA%\hermes\` and adds `oria` to your **User PATH** — open a new terminal after it finishes.
...
| `-SkipSetup` | off | Skip the post-install `oria setup` wizard |
...
A thin GUI installer is also available — useful if you'd rather double-click an `.exe` than open PowerShell. Download Oria Desktop, run the installer, and on first launch the GUI calls `install.ps1` under the hood to provision Python (via `uv`), Node, PortableGit, and the rest of the dependency bootstrap described below. After the first run, the desktop app and the PowerShell-installed `oria` CLI share the same `%LOCALAPPDATA%\hermes\hermes-agent` install and `%LOCALAPPDATA%\hermes` data directory — switch between the GUI and the CLI freely.

Use the desktop installer when you want a familiar Windows install experience or you're handing Oria to a non-developer; use the PowerShell one-liner when you're already in a terminal.

### Dependency bootstrap (`dep_ensure`)

On first launch (and on demand when a missing tool is detected), Oria runs a small Python bootstrapper — `hermes_cli/dep_ensure.py` — that checks for and lazily installs the non-Python dependencies it needs. On Windows, the relevant ones are:

| Dependency | Why Oria needs it |
|---|---|
...
8. **Sets `HERMES_GIT_BASH_PATH`** to the resolved `bash.exe` so Oria finds it deterministically in fresh shells.
9. **Adds `%LOCALAPPDATA%\hermes\bin` to User PATH and sets `HERMES_HOME=%LOCALAPPDATA%\hermes`** — exposes the `oria` command (and points it at your data dir) after you open a new terminal. Only the `hermes.exe` / `hermes-acp.exe` launchers are copied into this `bin` directory; the full `venv\Scripts` is deliberately **not** placed on PATH so Oria never shadows your own `python` command.
10. **Runs `oria setup`** — the normal first-run wizard (model, provider, toolsets). Skip with `-SkipSetup`.
...
|---|---|---|
| CLI (`oria chat`, `oria setup`, `oria gateway`, …) | ✓ | ✓ |
| Interactive TUI (`oria --tui`) | ✓ | ✓ |
...
## How Oria runs shell commands on Windows

Oria's terminal tool runs commands through **Git Bash**, same strategy Claude Code uses. This sidesteps the POSIX-vs-Windows gap without rewriting every tool.
...
The installer sets `HERMES_GIT_BASH_PATH` explicitly so fresh PowerShell sessions don't have to re-discover. Override it if you want Oria to use a specific bash — for example, your system Git Bash or a WSL-hosted bash via a symlink.

**Pitfall:** MinGit's layout is different from the full Git-for-Windows installer — bash lives under `usr\bin\bash.exe`, not `bin\bash.exe`. Oria checks both. If you're manually unpacking a MinGit zip, make sure you pick the **non-busybox** variant (`MinGit-*-64-bit.zip`, not `MinGit-*-busybox*.zip`) — busybox builds ship `ash` instead of `bash` and most coreutils are missing.

## UTF-8 console on Windows

Python's default stdio on Windows uses the console's active code page (usually cp1252 or cp437). Oria's banner, slash-command list, tool feed, Rich panels, and skill descriptions all contain Unicode. Without intervention, any of that crashes with `UnicodeEncodeError: 'charmap' codec can't encode character…`.
...
Oria's Windows stdio shim now sets `EDITOR=notepad` as a default. Notepad ships with every Windows install and works as a blocking editor — `subprocess.call(["notepad", file])` blocks until the window closes.
...
The `--wait` flag on VS Code is critical — without it the editor returns immediately and Oria gets a blank buffer back.
...
`oria gateway install` on Windows uses **Scheduled Tasks** with a Startup-folder fallback — no admin required.
...
```powershell
oria gateway status        # Merged view: schtasks + Startup folder + running PID
oria gateway start         # Starts the scheduled task now
oria gateway stop          # Graceful SIGTERM equivalent (TerminateProcess via psutil)
oria gateway restart
oria gateway uninstall     # Removes schtasks entry, Startup shortcut, pid file
```

`oria gateway status` is idempotent — call it a thousand times in a row and it will never accidentally kill the gateway. (Pre-PR #21561 it silently did, via `os.kill(pid, 0)` colliding with `CTRL_C_EVENT` at the C level — see "process management internals" below if you care about the story.)

### Why not a Windows Service?

Services require admin rights to install and tie the gateway's lifecycle to machine boot, not user login. The typical Oria user wants: log in → gateway available, log out → gateway gone. Scheduled Tasks do exactly that without elevation. If you genuinely want a service, use `nssm` or `sc create` manually — but you probably don't.
...
| `%LOCALAPPDATA%\hermes\bin\` | The `oria` / `hermes-acp` launchers and Oria's managed `uv.exe` (the Python manager it uses for updates). |
...
- `shutil.which("agent-browser", path=...)` picks up the `.cmd` shim automatically — `CreateProcessW` can't execute an extensionless shebang, so Oria always resolves to the `.CMD` wrapper. Don't manually invoke the shebang script; always go through the `.cmd`.
- Playwright Chromium is auto-installed on first run (`npx playwright install chromium`). If installation fails, `oria doctor` surfaces it with a fix-it hint.

## Running Oria on Windows — practical notes
...
Oria honors both `$env:X` (process-scope) and User environment variables (permanent, set in System Properties → Environment Variables). Setting API keys in `%LOCALAPPDATA%\hermes\.env` (your `HERMES_HOME`) is the normal path — same as Linux:
...
| `EDITOR` / `VISUAL` | Your editor for `/edit` and `Ctrl-X Ctrl-E`. Oria defaults to `notepad` if both are unset. |
...
The `oria uninstall` CLI subcommand also handles the case where the schtasks entry was registered under a different task name (older installs) — it searches by install path rather than by hardcoded task name.
...
Consequence: any codepath that said "check if this PID is alive" via `os.kill(pid, 0)` on Windows was silently killing the target. Oria migrated every such site (14 across 11 files) to `gateway.status._pid_exists()`, which uses `psutil.pid_exists()` (which in turn uses `OpenProcess + GetExitCodeProcess` on Windows — no signals). If you're writing a plugin or patch, use `psutil.pid_exists()` directly or `gateway.status._pid_exists()` — never `os.kill(pid, 0)`.
...
You hit a shebang-script invocation that bypassed the `.cmd` shim. Oria resolves commands through `shutil.which(cmd, path=local_bin)` so PATHEXT picks up `.CMD` — if you're invoking the tool via a hardcoded path instead, switch to the `.cmd` variant (e.g., `npx.cmd`, not `npx`).
...
Chromium is auto-installed on first run. If the install failed (rate-limited GitHub, Playwright CDN hiccup), run `oria doctor` — it will surface the missing Chromium and print the exact `npx playwright install chromium` command to fix it.

**`agent-browser` fails with a weird Node version error.**
The installer provisions Node 26 at `%LOCALAPPDATA%\hermes\node` but your PATH may have an older system Node 18 first. Either move Oria's node dir earlier on PATH, or delete the system install if you don't use Node elsewhere.
...
- **[CLI Reference](../reference/cli-commands.md)** — every `oria` subcommand.
