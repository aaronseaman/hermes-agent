...
Connect Oria to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Oria — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Oria runs it in a
...
oria photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Oria` project on your account.
...
Photon uses the same authorization model as every other Oria
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Oria replies with a pairing code. Approve it with:

```bash
oria pairing approve photon <CODE>
```
...
By default Oria responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Oria` and
...
Send an iMessage to your assigned number and Oria will reply.
...
assigned iMessage line Oria uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run oria photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `oria photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `oria photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `oria photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Oria sends images, voice
  ...
  media.
- **Native polls are supported.** Oria sends poll content through
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `oria send`, the dashboard) reuse the sidecar the
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Oria is set
...
| `PHOTON_MENTION_PATTERNS` | Oria wake words    | JSON list / comma / newline regex patterns for group mentions |
