...
sidebar_position: 3
title: "Discord"
description: "Set up Oria as a Discord bot"
---

# Discord Setup

Oria integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the Oria pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.

Before setup, here's the part most people want to know: how Oria behaves once it's in your server.

## How Oria Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Oria responds to every message. No `@mention` needed. Each DM has its own session. |
| **Server channels** | By default, Oria only responds when you `@mention` it. If you post in a channel without mentioning it, Oria ignores the message. |
...
| **Threads** | Oria replies in the same thread. Mention rules still apply unless that thread or its parent channel is configured as free-response. Threads stay isolated from the parent channel for session history. |
| **Shared channels with multiple users** | By default, Oria isolates session history per user inside the channel for safety and clarity. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |
| **Messages mentioning other users** | When `DISCORD_IGNORE_NO_MENTION` is `true` (the default), Oria stays silent if a message @mentions other users but does **not** mention the bot. This prevents the bot from jumping into conversations directed at other people. Set to `false` if you want the bot to respond to all messages regardless of who is mentioned. This only applies in server channels, not DMs. |

:::tip
If you want a normal bot-help channel where people can talk to Oria without tagging it every time, add that channel to `DISCORD_FREE_RESPONSE_CHANNELS`.
:::

### Discord Gateway Model

Oria on Discord is not a webhook that replies statelessly. It runs through the full messaging gateway, which means each incoming message goes through:

1. authorization (`DISCORD_ALLOWED_USERS`)
2. mention / free-response checks
3. session lookup
4. session transcript loading
5. normal Oria agent execution, including tools, memory, and slash commands
6. response delivery back to Discord

That matters because behavior in a busy server depends on both Discord routing and Oria session policy.
...
So if Alice and Bob both talk to Oria in `#research`, Oria treats those as separate conversations by default even though they are using the same visible Discord channel.
...
Oria tracks running agents by session key.
...
3. Enter a name for your application (e.g., "Oria") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential Oria uses to log in as your bot. Still on the **Bot** page:
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the Oria gateway).

## Step 7: Find Your Discord User ID

Oria uses your Discord User ID to control who can interact with the bot. To find it:
...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` Oria. `"all"` — accept all bot messages. |
...
- If a message arrives inside a thread or forum post and that thread has no explicit entry, Oria falls back to the parent channel/forum ID.
...
Discord's WebSocket resume window can expire during a restart or network outage. Messages sent during that gap are not delivered as live gateway events. When this option is enabled, Oria scans a bounded set of configured channel and thread histories after Discord reconnects, then sends still-unhandled messages through the same authorization, mention, channel, deduplication, and dispatch path as live events.
...
When `true`: Alice and Bob talking in `#research` each have their own separate conversation with Oria. When `false`: the entire channel shares one conversation transcript and one running-agent slot.
...
Oria automatically registers installed skills as **native Discord Application Commands**. This means skills appear in Discord's autocomplete `/` menu alongside built-in commands.
...
No extra configuration is needed — any skill installed via `oria skills install` is automatically registered as a Discord slash command on the next gateway restart.

### Disabling Slash Command Registration

If you run multiple Oria gateways against the same Discord application (e.g. staging + production), only one of them should own the global slash-command registration — otherwise the last startup wins and the registrations flap. Turn slash registration off on the "follower" gateway:
...
Discord's per-upload size limit depends on the server's boost tier (25 MB free, up to 500 MB). If Oria gets an HTTP 413, the adapter falls back to a link pointing at the local cache path rather than failing silently.
...
Oria supports Discord voice messages:
...
- **Discord voice channels**: Oria can also join a voice channel, listen to users speaking, and talk back in the channel.

For the full setup and operational guide, see:
- [Voice Mode](/user-guide/features/voice-mode)
- [Use Voice Mode with Oria](/guides/use-voice-mode-with-hermes)
...
discord.py plays only one audio stream per connection, so Oria installs a software mixer on the outgoing stream that sums an ambient loop, acknowledgements, and TTS replies into that single stream — they overlap instead of cutting each other off.
...
- `voice_playback_timeout_seconds` is a floor, not a hard cap for long TTS. Oria probes the generated audio duration and waits for `duration + 30s` when that is longer than the configured floor.
...
Oria 0.18 intentionally fails closed on externally reachable adapters. A Discord bot with no `DISCORD_ALLOWED_USERS`, no `DISCORD_ALLOWED_ROLES`, no `DISCORD_ALLOWED_CHANNELS`, and no explicit allow-all flag will connect successfully but deny inbound users before normal message handling.

### "Privileged intents" / `PrivilegedIntentsRequired` error on startup

**Cause**: Oria requests privileged Gateway Intents that are not enabled for your bot in the Developer Portal. Discord then rejects the WebSocket connection. Oria always requests **Message Content Intent**. It also requests **Server Members Intent** when your allowlist uses usernames (not numeric IDs) or when `DISCORD_ALLOWED_ROLES` is set. Presence Intent is not required.
...
3. Click **Save Changes**, then restart the gateway (`oria gateway restart`).

The gateway log should name the exact intent(s) Oria requested. Until they are enabled, Discord will keep rejecting the connection — this is a portal configuration error, not a flaky network issue.
...
**Cause**: The Oria gateway isn't running, or the token is incorrect.

**Fix**: Check that `oria gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.
