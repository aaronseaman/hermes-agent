...
Connect Oria to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Oria supports two WeCom integration modes:
...
> Run `oria gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Oria decrypts the message, queues it for the agent
...
(Use `oria gateway start` only after `oria gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Oria and the token the
...
fixing, restart `oria gateway run`.
...
Check `oria gateway run` logs for the bound host/port. If the adapter bound to
