...
description: "Set up Oria as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Oria can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Oria gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not an Oria limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Oria degrades gracefully if absent.
...
## Exposing Oria to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Oria gateway has to be reachable from Meta's servers.  Three common ways:
...
3. Start the Oria gateway: `oria gateway`.
...
The `oria whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.
...
When the agent invokes any of these flows, Oria uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Oria acknowledges inbound messages immediately:
...
Oria TTS produces MP3. Two paths:
...
Oria warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Oria. If you need it, please [open an issue](https://github.com/NousResearch/hermes-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Oria's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Oria doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Gateway not running** — check `oria gateway` is up.
- **App Secret not set** — without it, Oria refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Oria.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `oria tools list` and verify the platform is using `hermes-whatsapp` (the default Cloud adapter toolset, same as Baileys).
...
The default `stt.provider: local` requires `pip install faster-whisper`.  If you're a Nous subscriber, you can route STT through the managed gateway instead — select **Nous Subscription** for speech-to-text in `oria tools`, or set it directly:

```bash
oria config set stt.provider nous
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Oria will accept as authentic.
...
| | Baileys (`oria whatsapp`) | Cloud API (`oria whatsapp-cloud`) |
|---|---|---|
...
Most users running Oria for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
