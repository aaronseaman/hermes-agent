...
sidebar_position: 4
title: "Slack"
description: "Set up Oria as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Oria to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Oria instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Oria uses the modern
...
The fastest path is to paste a manifest Oria generates for you. It
...
1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   oria slack manifest --agent-view --write
   ```
   ...
   ```bash
   oria slack manifest --agent-view \
...
4. Enter an app name (e.g., "Oria") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Oria can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not an Oria configuration issue.
:::
...
Oria uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
oria gateway setup      # Select Slack when prompted
```

Then start the gateway:

```bash
oria gateway                # Foreground
oria gateway install        # Install as a user service
sudo oria gateway install --system     # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Oria now
...
Every Oria command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Oria command with its description.

Under the hood: Oria ships with a generated Slack app manifest (see
...
New Slack apps use Slack's **Agent** messaging experience. Existing Oria
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Oria can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Oria only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Oria adds new commands (e.g. after `oria update`), regenerate
the manifest and update your Slack app:

```bash
oria slack manifest --write
```
...
`/hermes bg run the tests` — Oria routes it the same way as `/bg
...
Slack never delivers them to Oria.

As a workaround, Oria recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Oria treats it identically to the slash form and replies in the same
thread.
...
Oria falls back to a text prompt, the prompt instructs you to reply
...
fails, Oria retries via a second ephemeral API path — a slash reply is never
...
```bash
oria slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Oria behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Hermes Agent what time is it?`). In channels, Oria replies in a thread attached to that message. |
| **Threads** | If you @mention Oria inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions Oria, and "all" accepts every
      # other bot. Oria always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.native_task_cards` | `false` | When `true`, renders live tool calls as Slack-native plan/task cards. Cards work with Slack's built-in default `tool_progress: off`; an explicitly configured `display.tool_progress: off` (global or `display.platforms.slack`; `/verbose` writes the same key) disables cards too. Cards need a thread: when the card lane is active and the chat has no thread to anchor on (a top-level DM with `reply_in_thread: false`), Oria shows no tool progress instead of text bubbles, unless you explicitly set `tool_progress: new`/`all`, which falls back to editable text progress there. Recoverable native API failures fall back to one continuously edited text update. |
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions Oria, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default Oria sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Oria is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), Oria uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — Oria seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and Oria falls back
...
- Oria checks thread eligibility before attempting publication, so a
...
- On a supported threaded destination, if the native stream fails for a
  recoverable reason (API error, rate limit), Oria falls back to a single continuously edited text message so
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Oria in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Oria will respond.
...
By default Oria ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Oria instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Oria always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer Oria agents are filtered consistently across workspaces.
...
`client_msg_id` — the same signature Oria uses to recognise app posts — so it
is dropped as bot traffic. This blocks a common pattern: a custom front-end (an
internal dashboard, a mobile shell, a kiosk) that sends messages to Oria *as*
the logged-in user.
...
manifest — regenerate with `oria slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Oria will deliver scheduled messages,
...
Delivery works even when the cron process isn't co-located with the gateway — Oria falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
Oria can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Oria also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Oria uses the correct workspace-specific client to respond.
...
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Oria now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
- Audit who has access to your Oria config directory
