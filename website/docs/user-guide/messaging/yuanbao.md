...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Oria to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Oria to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
All standard Oria commands work on Yuanbao:
...
4. Consider enabling verbose logging: `oria gateway run -vv`
...
Yuanbao has a maximum message size. Oria automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
oria chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
