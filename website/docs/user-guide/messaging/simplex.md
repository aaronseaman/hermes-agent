...
> Run `oria gateway setup` and pick **SimpleX** for a guided walk-through.
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `oria pairing approve simplex <CODE>`.
...
`simplex:group:12` as a cron `deliver=` target or in a `oria send` call.

## Sending with `oria send`

SimpleX works as a standalone send target — the daemon must be running,
but a live gateway is not required for plain text:

```bash
oria send --to simplex:alice "hello"            # DM by contact display name
oria send --to simplex:group:12 "hello"         # group by numeric ID
oria send --to simplex "hello"                  # SIMPLEX_HOME_CHANNEL
```
...
`oria send --list` shows them by name. Before the first gateway run the
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`oria send` CLI](/guides/pipe-script-output):

```bash
oria send simplex:<contact-id> "Done!"
```

## Privacy notes

- SimpleX never reveals phone numbers or email addresses — contacts use opaque IDs
- The connection between Oria and the daemon is local WebSocket (`ws://127.0.0.1:5225`) — no data leaves your machine
