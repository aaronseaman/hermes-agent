...
title: "SMS (Twilio)"
description: "Set up Oria as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Oria connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
If you're running Oria locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Oria will respond via SMS.
...
Oria validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Oria gateway logs for Twilio API errors
