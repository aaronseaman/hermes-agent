...
ntfy makes a great lightweight push channel for Oria: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `oria gateway setup` and pick **ntfy** for a guided walk-through.
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already an Oria dependency.
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Oria adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`oria send` CLI](/guides/pipe-script-output):

```bash
oria send ntfy:alerts-channel "Done!"
```
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Oria replies:
