...
description: Control your smart home with Oria via Home Assistant integration.
...
Oria integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Oria"
5. Copy the token
...
Oria registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Oria".
...
from `config.yaml`. Double-check the file lives under the active Oria profile
...
running Oria — `curl -H "Authorization: Bearer <token>" <url>/api/` should
