...
Oria can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; Oria does the rest.
...
2. Click **Install runtime**. Oria downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with Oria, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How Oria picks what to download
...
Models ship in several quality grades (quantizations). Oria picks the
...
guarantees. Below 4-bit the quality loss is too severe, so Oria never
...
Local models live or die by memory placement, so Oria manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, Oria deliberately places the
...
- **Memory fit includes the launch configuration**, not just the model file:
  ...
  count. For multi-token prediction (MTP), Oria uses smaller batches when
...
- **Conversation compression follows a growth check.** If a larger window
  ...
  Oria compresses instead of claiming a window the server did not receive.
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — Oria
...
If a llama-server is already running on your machine, Oria detects it
...
[Run Oria Locally with Ollama](/guides/local-ollama-setup) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with Oria.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; Oria updates it with
                     # each release after re-validation
```

Models and runtime builds live under the Oria home directory
