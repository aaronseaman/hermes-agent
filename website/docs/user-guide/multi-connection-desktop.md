...
# Connecting Desktop to Many Oria Instances

Register every Oria backend you own — the local runtime, remote gateways on
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Oria gateway…"**) deep-links straight to
...
list of Oria gateways. Its intro says it plainly: *"Manage this device and
every Oria gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Oria runtime managed by this app." | automatic |
| **Remote gateway** | "An Oria gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "An Oria install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
...
4. Fill the fields:
   ...
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `oria serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Oria agents
...
connection is registered) dispatches `oria update` to every eligible
...
panel, ⌘K **Update Oria**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `oria serve` is running on the remote host, the
...
- **"Update Oria Desktop to chat with agents on other connections"** — the
