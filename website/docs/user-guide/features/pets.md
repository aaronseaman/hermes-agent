...
Oria can show an animated **pet** — a small mascot sprite that reacts to what
...
In the terminal (CLI/TUI), Oria renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `oria pets list [query] [--limit N]` |
| List installed pets | `oria pets list --installed` |
| Install a pet | `oria pets install <slug> [--select] [--force]` |
| Set the active pet | `oria pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `oria pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `oria pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `oria pets off` |
| Remove an installed pet | `oria pets remove <slug>` |
| Diagnose setup | `oria pets doctor` |

`oria pets show` flags:
...
Beyond installing pre-made pets from the gallery, Oria can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `oria pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `oria tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
...
always-on-top desktop window. Out there it stays visible while Oria is
...
Run `oria pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Oria uses its own
  profile-scoped `<HERMES_HOME>/pets/` instead — install through `oria pets`.
