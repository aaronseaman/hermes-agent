...
**Tool Search** is Oria's opt-in progressive-disclosure layer for that
...
:::info Built-in Oria tools never defer
The tools that make up Oria's core capability set (`terminal`,
...
When the model invokes `tool_call`, Oria **unwraps the bridge** and
...
- **No provider-native validation for deferred schemas.** `tool_describe`
  ...
  only the generic `tool_call.arguments` object. Oria therefore coerces and
  validates the underlying arguments locally before dispatch; the concrete
  tool or MCP server remains responsible for schemas Oria cannot safely
...
- **No JS sandbox.** Oria uses the simpler "structured tools" mode
