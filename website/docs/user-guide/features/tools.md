...
description: "Overview of Oria's tools — what's available, how toolsets work, and terminal backends"
---
...
Oria ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, scheduled tasks, Home Assistant, and more.
...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `oria tools` → 🐦 X (Twitter) Search. |
...
| **Automation** | `cronjob` | Scheduled tasks with create/list/update/pause/resume/run/remove actions. Outbound delivery is handled by cron's own delivery, the `oria send` CLI, and the gateway notifier — not by an agent-callable tool. |
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `oria model` to enable it, or configure individual tools with `oria tools`.
:::
...
oria chat --toolsets "web,terminal"
...
**One persistent container, shared across the whole process.** Oria starts a single long-lived container on first use (`docker run -d ... sleep infinity`) and routes every terminal, file, and `execute_code` call through `docker exec` into that same container. Working-directory changes, installed packages, environment tweaks, and files written to `/workspace` all carry over from one tool call to the next, across `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the Oria process. The container is stopped and removed on shutdown.
...
# Configure
oria config set terminal.backend singularity
oria config set terminal.singularity_image ~/python.sif
```
...
modal setup
oria config set terminal.backend modal
```
...
oria config set terminal.backend vercel_sandbox
oria config set terminal.vercel_runtime node24
```

Authenticate with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This access-token setup is the supported path for deployments and normal long-running Oria processes on Render, Railway, Docker, and similar hosts. Supported runtimes are `node24`, `node22`, and `python3.13`; Oria defaults to `/vercel/sandbox` as the remote workspace root.
...
Oria keeps the newest **64 completed results**, for up to **7 days after
completion**, under `logs/process-results/` in the profile's Oria home. Each
