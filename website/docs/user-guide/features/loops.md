...
2. **First wakeup fires right away** — on the next idle poll (gateway: the next 15s watcher scan), Oria injects the wakeup and runs a normal turn against current state.
...
Works on the CLI, the TUI (`oria --tui`), the web dashboard chat, the desktop app, and every gateway platform (Telegram, Discord, Slack, WhatsApp, …). On messaging platforms the gateway fires wakeups even between your messages — the loop belongs to the chat's session, and its results arrive as ordinary replies — also when that session is open in the TUI / Desktop app, which leaves the routed wakeup to the gateway.
