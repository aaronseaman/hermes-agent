...
# Optional: only if a browser must call Oria directly
...
- **Chat Completions**: Oria emits `event: hermes.tool.progress` for tool-start visibility without polluting persisted assistant text.
- **Responses**: Oria emits spec-native `function_call` and `function_call_output` output items during the SSE stream, so clients can render structured tool UI in real time.
...
Tool calls in the `output` array were already executed server-side by the Oria agent — they are replayed with `"status": "completed"` for structured tool UI, never as pending calls for the client to execute.
...
enumerate every authenticated provider/model combination Oria can route to,
...
Normal opens are intentionally conservative for custom providers: Oria probes
...
Use this endpoint when integrating dashboards, browser UIs, or control planes so they can discover whether the running Oria version supports runs, streaming, cancellation, and session continuity without depending on private Python internals.

## Browser-extension control

Oria can route browser tools through an authenticated extension that controls
the browser session associated with the current Oria session. The feature is
...
register only for an existing server session. Oria derives the controller
...
Oria preserves the existing browser backend. Once the gateway binds a
...
Oria never retries the same action through another backend.
...
2. Oria returns a single-use ticket with a 30-second TTL and the filtered,
...
Oria sends `browser.controller.command` frames containing `command_id`,
...
- `provider` — the Oria provider slug to resolve credentials/runtime for this turn
...
alias, Oria rejects the request with `400` instead of silently remixing route
...
Runs accept a simple `input` string and optional `session_id`, `instructions`, `conversation_history`, or `previous_response_id`. When `session_id` is provided, Oria surfaces it in the run status so external UIs can correlate runs with their own conversation IDs.

For safely retryable creation, send an `Idempotency-Key` header (1–255 visible ASCII characters). Oria durably reserves the key before starting work. An identical retry returns the original `run_id` with HTTP 202 and `Idempotency-Replayed: true`, including after a gateway restart and after the run has completed, failed, or been cancelled. Reusing the same key with a different JSON payload returns HTTP 409 with code `idempotency_key_conflict`. Keys are isolated by authenticated API profile/credential and retained for 24 hours after their last status update; clients should use unique, unguessable keys and must not reuse them for unrelated operations. Requests without the header retain the legacy behavior and always create a new run.

When `session_id` identifies an existing Oria session and no explicit
...
Interrupt a running agent turn. The endpoint returns immediately with `{"status": "stopping"}` while Oria asks the active agent to stop at the next safe interruption point.
...
Create a new scheduled job. Body accepts the same shape as `oria cron` — prompt, schedule, skills, provider override, delivery target.
...
External UIs can manage Oria sessions over REST without standing up the dashboard. All endpoints are gated by `API_SERVER_KEY` and live under `/api/sessions/*`.
...
Rules: max 256 chars, control characters (`\r`, `\n`, `\x00`) are rejected, and the value is echoed back on responses (JSON + SSE). `/v1/capabilities` advertises support via `"session_key_header": "X-Hermes-Session-Key"`. Without the key, Honcho's `per-session` strategy produces a different scope per `session_id` — exactly the behavior Oria had before.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call Oria directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.
...
To give multiple users their own isolated Oria instance (separate config, memory, skills), use [profiles](/user-guide/profiles):

```bash
# Create a profile per user
oria profile create alice
oria profile create bob
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated Oria instance. See the [Open WebUI guide](/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
