...
You can also point Oria at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `oria update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:
...
```bash
oria profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
oria skills opt-out              # stop future seeding — nothing on disk is touched
oria skills opt-out --remove     # also delete UNMODIFIED bundled skills (confirms first)
oria skills opt-in --sync        # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `oria update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `oria skills opt-in`) to re-enable.

:::note Safe by default
`oria skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Oria installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
(Plan mode works the same way but is a built-in command now: `/plan [request]` tells Oria to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hermes/plans/` relative to the active workspace/backend working directory.)

You can also interact with skills through natural conversation:

```bash
oria chat --toolsets skills -q "What skills do you have?"
oria chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, Oria asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `oria setup` or `~/.hermes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `oria config migrate` prompts for unconfigured settings, and `oria config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
and `examples/`. Unreferenced repository files are not copied. Oria scans the
...
policy above), Oria can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of Oria — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Oria to scan those directories too.
...
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Oria process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Oria ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
Paths support `~` expansion and `${VAR}` substitution; relative paths resolve against your Oria home. Setting `create_dir` to the local skills dir is the same as leaving it unset.
...
Skills are procedure documents the agent follows, so Oria does **not** auto-load them from arbitrary cloned repos. The first time you run Oria in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `oria skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
oria skills trust               # trust the current repo
oria skills trust ~/myproject # or explicitly
oria skills untrust             # revoke
```
...
- `description` (optional) — short text shown in `/bundles` and `oria bundles list`.
...
oria bundles create research

# Overwrite an existing bundle
oria bundles create backend-dev --skill ... --force
...
```bash
oria skills browse                                # Browse all hub skills (official first)
oria skills browse --source official              # Browse only official optional skills
oria skills search kubernetes                     # Search all sources
oria skills search react --source skills-sh       # Search the skills.sh directory
oria skills search https://mintlify.com/docs --source well-known
oria skills inspect openai/skills/k8s             # Preview before installing
oria skills install openai/skills/k8s             # Install with security scan
oria skills install official/security/1password
oria skills install skills-sh/vercel-labs/json-render/json-render-react --force
oria skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
oria skills list --source hub                     # List hub-installed skills
oria skills check                                 # Check installed hub skills for upstream updates
oria skills update                                # Reinstall hub skills with upstream changes when needed
oria skills audit                                 # Re-scan all hub skills for security
oria skills uninstall k8s                         # Remove a hub skill
oria skills reset google-workspace                # Un-stick a bundled skill from "user-modified" (see below)
oria skills reset google-workspace --restore      # Also restore the bundled version, deleting your local edits
oria skills publish skills/my-skill --to github --repo owner/repo
oria skills snapshot export setup.json            # Export skill config
oria skills tap add myorg/skills-repo             # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Oria. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `oria skills search <query> --source skills-sh`. Oria resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Oria currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Oria repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Oria can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
oria skills search https://mintlify.com/docs --source well-known
oria skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
oria skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Oria can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
oria skills install openai/skills/k8s
oria skills tap add myorg/skills-repo
```
...
- Oria source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Oria can search and convert agent entries from LobeHub's public catalog into installable Oria skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Oria source id: `lobehub`
...
- Oria source id: `browse-sh`
- Trust level: `community`

```bash
oria skills search airbnb --source browse-sh
oria skills inspect browse-sh/airbnb.com/search-listings-ddgioa
oria skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Oria also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Oria source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Oria does not enumerate or copy unrelated files from the host.

```bash
oria skills install https://sharethis.chat/SKILL.md
oria skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
oria skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `oria skills update` re-fetches from the same URL automatically when you want to refresh.
...
`oria skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Oria | Always trusted |
...
```bash
oria skills check            # Report which installed hub skills changed upstream
oria skills update           # Reinstall only the skills with updates available
oria skills update react     # Update one specific installed hub skill
oria skills update react --force     # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Checks skip network requests for missing or non-directory installs (`orphaned`) and unsafe or unresolvable recorded paths (`invalid_install`). Missing-directory entries can be removed with `oria skills uninstall <name>`; invalid paths require inspecting and repairing the active profile's `skills/.hub/lock.json` before retrying. No entries are removed automatically.
...
Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `oria skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Oria users add with `oria skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Oria discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Oria user can subscribe and install:

```bash
oria skills tap add my-org/hermes-skills
oria skills search deploy
oria skills install my-org/hermes-skills/deploy-runbook
```
...
The `oria skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `oria skills tap list` shows the effective path per tap.
...
```bash
oria skills install owner/repo/skills/my-workflow
```
...
## Bundled skill updates (`oria skills reset`)

Oria ships with a set of bundled skills in `skills/` inside the repo. On install and on every `oria update`, a sync pass copies those into `~/.hermes/skills/` and records a manifest at `~/.hermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Oria recomputes the hash of your local copy and compares it to the origin hash:
...
Generated runtime caches inside a skill (`__pycache__/`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`, and a `.pyc` sitting next to its `.py`) are not part of the hash, so running a skill's helper script never marks it user-modified or hides it from `oria skills list-modified` / `diff`.
...
`oria skills reset` is the escape hatch:
...
oria skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `oria -p coder skills reset <name>` only affects that profile.
:::
