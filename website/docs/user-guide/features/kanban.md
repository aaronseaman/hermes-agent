...
description: "Durable SQLite-backed task board for coordinating multiple Oria profiles"
---
...
Oria Kanban is a durable task board, shared across all your Oria profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.hermes/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
oria kanban boards list

# Create a new board.
oria kanban boards create atm10-server \
    --name "ATM10 Server" \
...
oria kanban --board atm10-server list
oria kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
oria kanban boards switch atm10-server
oria kanban boards show               # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
oria kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.hermes/kanban/current` — the slug persisted by `oria kanban
   boards switch`.
4. `default`.
...
`oria dashboard` → Kanban tab shows a board switcher at the top as soon
...
oria kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `oria kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `oria gateway
...
until one comes up — `oria kanban create` warns about this at creation
time.

Running `oria kanban daemon` as a separate process is **deprecated**;
...
Mounting one `kanban.db` in several Oria homes (containers, fleet hosts) shares
...
oria kanban create "nightly ops review" \
    --assignee ops \
...
```bash
oria kanban complete t_abc t_def t_hij --result "batch wrap"
oria kanban archive    t_abc t_def t_hij
oria kanban unblock    t_abc t_def
oria kanban block      t_abc "need input" --ids t_def t_hij
```
...
```bash
oria -p planner tools enable kanban                        # CLI / TUI / Desktop chats
oria -p planner tools enable kanban --platform telegram    # a gateway platform
```

Each platform has its own selection under `platform_toolsets.<platform>` in
`config.yaml`; the toolset is also a checkbox in `oria tools` and the dashboard.
...
**Workers do not shell out to `oria kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `oria kanban` CLI.
...
**Zero schema footprint on normal sessions.** A regular `oria chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
**Agent-side prevention:** Before the worker exits, Oria injects up to two
...
**Worker session names.** A worker's session is titled after its card (`Fix the swap modal`, or `Kanban task <id>` when the board row can't be read) at spawn, so `oria sessions` and session search show the card, not a model's guess. Workers never make the auxiliary `title_generation` model call that names interactive sessions; a manual `/title` in a worker session still wins.
...
```bash
oria kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

oria kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `oria skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
oria kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
oria kanban set-model t_abcd claude-opus-4.6 --provider anthropic
oria kanban set-model t_abcd none      # clear the override
```
...
For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `oria kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
oria kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `oria kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `oria kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `oria profile create --description "..."`, `oria profile describe <name> --text "..."`, `oria profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `oria profile describe --auto`). |
...
If you run `oria dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.hermes/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `oria -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard Oria dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
oria kanban init                                       # create kanban.db + print daemon hint
oria kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
oria kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
oria kanban show <id> [--json]
oria kanban assign <id> <profile>                      # or 'none' to unassign
oria kanban reassign <id>... <profile>                 # bulk re-assign tasks to a profile
oria kanban edit <id> [--title ...] [--body ...]       # edit task title / body / priority in place
        [--priority N]
oria kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
oria kanban schedule <id> --at <ISO8601>               # set/clear a task's scheduled_at start time
oria kanban diagnostics [--json]                       # board health snapshot (alias: diag)
oria kanban link <parent_id> <child_id>
oria kanban unlink <parent_id> <child_id>
oria kanban claim <id> [--ttl SECONDS]
oria kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
oria kanban complete <id>... [--result "..."] [--force]
oria kanban block <id> "<reason>" [--ids <id>...]
oria kanban unblock <id>...
oria kanban archive <id>...

oria kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
oria kanban request-changes <id> "<required changes>"                 # active reviewer -> implementer
oria kanban reopen-review    <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

oria kanban tail <id>                                  # follow a single task's event stream
oria kanban watch [--assignee P] [--tenant T]          # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
oria kanban heartbeat <id> [--note "..."]              # worker liveness signal for long ops
oria kanban runs <id> [--json]                         # attempt history (one row per run)
oria kanban assignees [--json]                         # profiles on disk + per-assignee task counts
oria kanban dispatch [--dry-run] [--max N]             # one-shot pass
        [--failure-limit N] [--json]
oria kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `oria gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
oria kanban stats [--json]                             # per-status + per-assignee counts
oria kanban log <id> [--tail BYTES]                    # worker log from ~/.hermes/kanban/logs/
oria kanban notify-subscribe <id>                      # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
oria kanban notify-list [<id>] [--json]
oria kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
oria kanban context <id>                               # what a worker sees
oria kanban specify [<id> | --all] [--tenant T]        # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
oria kanban gc [--event-retention-days N]              # workspaces + old events + old logs
...
```bash
oria kanban create "nightly backup audit" \
...
To see why a ready card is not spawning, run `oria kanban dispatch --dry-run` — the output lists `Guarded (<reason>): <task id>` per held card (and `respawn_guarded`, `rate_limited`, `skipped_locked`, `memory_pressure` with `--json`). The gateway's and the standalone daemon's "dispatcher stuck" warning also names what the last tick held back, e.g. `Last tick held back: active_pr=1`.
...
Every `oria kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `oria chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `oria kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `oria kanban`. You don't have to leave the chat to drive the board.
...
Dispatcher workers creating tasks through `kanban_create` or `oria kanban create`
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`oria kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `oria kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
oria kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
```bash
oria kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
oria kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
oria kanban notify-list
oria kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
oria kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
oria kanban runs t_abcd
...
**Bulk close caveat.** `oria kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.

**Live-claim guard on complete.** A `running` task whose worker holds a live claim is only completed by that worker (`kanban_complete` from inside the run) or by an explicit operator override: `oria kanban complete <id> --force` and the dashboard's "mark done" action. A claim-less `oria kanban complete <id>` or an orchestrator session's `kanban_complete` is refused with a pointer to `--force` / `oria kanban reclaim`, so a second session can no longer close a live worker's run underneath it. Completing `ready`, `blocked` or `review` cards without a claim is unchanged.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`oria kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `oria kanban heartbeat $TASK` to signal liveness during long operations. |
...
`oria kanban tail <id>` shows these for a single task. `oria kanban watch` streams them board-wide.
