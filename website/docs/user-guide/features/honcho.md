...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Oria's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Oria instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
oria memory setup      # select "honcho" from the provider list
```
...
- `per-session` — each `oria` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Oria syncs them back at session init.
...
The `oria honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `oria memory setup honcho` (or run `oria memory setup` and pick it from the list); the `oria honcho` subcommand then appears on the next invocation.

```bash
oria memory setup honcho      # Configure Honcho directly (works before activation)
oria honcho status            # Connection status, config, and key settings
oria honcho setup           # Redirects to `oria memory setup` (post-activation alias)
oria honcho strategy          # Show or set session strategy (per-session/per-directory/per-repo/global)
oria honcho peer              # Show or update peer names + dialectic reasoning level
oria honcho mode              # Show or set recall mode (hybrid/context/tools)
oria honcho tokens            # Show or set token budget for context and dialectic
oria honcho identity          # Seed or show the AI peer's Honcho identity
oria honcho sync              # Sync Honcho config to all existing profiles
oria honcho peers             # Show peer identities across all profiles
oria honcho sessions          # List known Honcho session mappings
oria honcho map               # Map current directory to a Honcho session name
oria honcho enable            # Enable Honcho for the active profile
oria honcho disable           # Disable Honcho for the active profile
oria honcho migrate           # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `oria honcho`

If you previously used the standalone `oria honcho setup`:
...
No re-login or re-setup needed. Run `oria memory setup` and select "honcho" — the wizard detects your existing config.
