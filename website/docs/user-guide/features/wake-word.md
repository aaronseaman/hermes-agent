...
The wake word turns Oria into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Oria listens in the background for a spoken
trigger phrase. Say it, and Oria starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Oria backend (for example a
...
Oria supports **client capture** for that case:
...
By default the phrase is **"hey hermes"** — a model for it ships with Oria, so
...
```bash
# In an interactive `oria` session:
...
default) to let Oria choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Oria installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Oria surfaces, and `surface` picks which one
...
command (`oria -p <profile>`) instead of routing.
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `oria tools` (Voice section) sets them up.
...
but the phrase never fires. Oria detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Oria
backend (it may appear as your terminal, `python`, or Oria), then toggle the
...
```bash
oria config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
oria config set wake_word.input_device null
```
