...
| Use case | "Use Oria as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Oria stores
the refresh token in `~/.hermes/auth.json` — the same place all Oria
...
If you see `not logged in`, run `oria portal`. If you see
