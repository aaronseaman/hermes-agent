...
fills the password through Oria. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Oria generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `oria vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Oria
...
in, Oria picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Oria hands the master password to the manager's
...
Prefer not to use a detected manager? `oria vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `oria vault add`), bound to the checkout site,
...
- **CLI**: `oria vault list`, `oria vault add`, `oria vault rm <handle>`,
  `oria vault sources`.
...
Cron jobs, webhooks, the API server and `oria chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Oria: not in
