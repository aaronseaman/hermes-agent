...
description: "How Oria remembers across sessions — MEMORY.md, USER.md, and session search"
---

# Persistent Memory

Oria has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
:::caution One agent per Oria home
Don't point two agent processes at the same Oria home directory. Memory writes are automatic and load back into the system prompt at session start, so two writers sharing one home will compound each other's entries into state neither of them (nor you) authored. Memory is scoped per [profile](/user-guide/profiles) by design — give a second agent its own profile, and if they need shared memory, use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
```bash
oria sessions list      # Browse past sessions
```
...
The learning journey is a timeline view of everything Oria has learned — saved skills and memory entries plotted over time (oldest at top, newest at bottom), with a playable "constellation" scrubber that replays the build-up. The same graph data drives three surfaces:

- **Classic CLI / standalone** — `oria journey` (aliases: `oria learning`, `oria memory-graph`) renders the timeline in the terminal. Flags: `--play` animates the build-up (`--fps` to tune it), `--width`/`--height` override the render size, `--no-color` disables color, and `--json` dumps the raw graph payload.
...
Beyond viewing, the journey is also where you **prune and correct** what Oria has learned:

| Command | What it does |
|---------|--------------|
| `oria journey list` | List node ids — skill names and `memory:<source>:<index>` ids for memory chunks. |
| `oria journey delete <node> [-y]` | Delete a node. Skills are **archived** (restorable), memory chunks are removed. `-y` skips the confirmation. |
| `oria journey edit <node>` | Open the node's content (a skill's `SKILL.md` or the memory chunk) in `$EDITOR`. |
...
or update a skill. This is Oria's consent-aware learning loop: repeated
...
For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Oria ships with 8 external memory provider plugins — including Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.

External providers run **alongside** built-in memory (never replacing it) and add capabilities like knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.

```bash
oria memory setup        # pick a provider and configure it
