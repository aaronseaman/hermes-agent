...
A **profile distribution** packages a complete Oria agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Oria has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `oria profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `oria profile update` pulls new versions | Re-send the file |
...
| Made with | `oria profile install` / `update` | `/export` and `/import`, or `oria profile export` / `import` |
...
Before distributions, sharing an Oria agent meant sending someone:
...
```bash
oria profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `oria profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Oria in 2026, that's already true.
...
:::caution
**Oria does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `oria profile install` or `oria profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
oria profile create research-bot
...
Recipients who run `oria profile update research-bot` will pull the latest.
...
```bash
oria profile install github.com/you/research-bot --alias
```
...
```bash
# GitHub shorthand
oria profile install github.com/you/research-bot

# Full HTTPS
oria profile install https://github.com/you/research-bot.git

# SSH
oria profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
oria profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
oria profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
oria profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
oria profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
oria profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Oria distribution.
...
```bash
oria profile info research-bot
```
...
`oria profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
oria profile update research-bot
```
...
```bash
oria profile delete research-bot
```
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `oria profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
oria profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `oria profile update pr-reviewer` and everyone's on the new version within minutes.
...
# Anyone
oria profile install github.com/you/hermes-polymarket-trader --alias
```
...
# Each on-call
oria profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
oria profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `oria profile info <name>` and hold off on updates until you're ready.
:::
...
oria profile info research-bot | grep Version
...
oria profile delete research-bot --yes
oria profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
oria profile update research-bot --force-config --yes
```
...
oria profile install github.com/yourname/forked-research-bot --alias
...
oria profile install ~/.hermes/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
oria profile delete research-bot-test --yes
oria profile install ~/.hermes/profiles/research-bot --name research-bot-test
```
...
Without `-o`, the CLI and TUI place the archive in Oria's managed
`profile-exports/` directory under the default Oria home, not in the current
...
source file. If the Oria home itself lives inside a Git checkout (some
...
```bash
oria profile export research-bot
oria profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
oria profile import ./research-bot.tar.gz
oria profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `oria -p research-bot`) is created when the name doesn't collide with an existing command.
...
**The default profile** (`~/.hermes`) is exported through an allow-list — only known Oria artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `oria -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `oria profile` — not a parallel command tree.
...
- Reserved profile names (`oria`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`oria profile export` / `import`](../reference/profile-commands.md#oria-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Oria](../guides/use-soul-with-hermes.md) — authoring personalities
