...
When Oria runs your agent inside a Docker terminal sandbox, that sandbox normally holds your real upstream API keys (`OPENROUTER_API_KEY`, `OPENAI_API_KEY`, etc.). A prompt-injected agent in that sandbox can `cat ~/.config/openrouter/auth.json` or `printenv | grep -i key` and exfiltrate them.
...
- It is **not** the inbound `oria proxy` command, which is an OAuth aggregator reverse proxy. Different command (`oria egress`), different direction.
...
`oria egress setup` discovers provider keys from your environment. If your keys live only in `~/.hermes/.env` (not exported into your shell), setup reads that file automatically — you don't have to `export` them first.

When you re-run `setup` later (new allowlist host, rotated tokens, switched credential source), it stops the running daemon because its config is held in memory, then **offers to restart it for you** so the change takes effect immediately. On a tty it asks; pass `--restart` to always restart or `--no-restart` to leave it down. To apply changes any other time, `oria egress restart` is the one-command stop-then-start.
...
These env vars are present on most developer laptops for unrelated tooling (terraform, gcloud, aws CLI, ECR push). They surface as warnings in the wizard and `oria egress status` but never block the proxy from starting. If you don't use those providers from sandboxes, `unset` the vars to clear the warning.

## Bitwarden integration

If you already use Bitwarden Secrets Manager via [`oria secrets bitwarden setup`](../secrets/bitwarden), the egress proxy can pull real credentials from there instead of `os.environ`:

```bash
oria egress setup --from-bitwarden
```
...
2. `oria egress stop && oria egress start` on the host.
3. Sandboxes started after that point swap proxy tokens for the new value.

No `.env` edits. No Oria restart on the host. The proxy daemon is the only thing that touches the new value — your host process and `os.environ` are untouched.

### Fail-loud at start

When `credential_source: bitwarden`, `oria egress start` pre-checks at the wizard layer AND `_build_proxy_subprocess_env` re-checks at the daemon layer:

- BWS access token env var is unset → refuse to start with a hint to `unset` and re-run, or `oria egress setup --no-bitwarden` to switch back to env mode
- `secrets.bitwarden.project_id` is empty → refuse to start with a hint to run `oria secrets bitwarden setup`
...
oria egress install --force            # re-download even if a managed copy exists

oria egress setup                      # interactive wizard
oria egress setup --tunnel-port N      # override the tunnel listener port
oria egress setup --from-bitwarden     # use BWS as credential source (fail-loud)
oria egress setup --no-bitwarden       # explicitly switch back to env mode
oria egress setup --rotate-tokens      # mint fresh tokens for every provider
...
oria egress restart                    # stop (if running) then start — needed when
                                       #   upstream SECRETS change (rotation, new provider)
oria egress reload                     # hot-reload the ruleset from proxy.yaml via the
                                       #   management API — no restart, no dropped
                                       #   connections (allowlist / mapping edits)

oria egress status                     # binary + config + pid + listening state + mappings
oria egress status --show-tokens       # print proxy tokens in full
                                       #   (default: redacted prefix + suffix only)

oria egress disable                    # flip proxy.enabled = false
                                       #   (does not stop a running proxy)

oria egress config                     # print the path to proxy.yaml for debugging
```

### Token rotation

By default, `oria egress setup` **preserves** proxy tokens for providers that already have them. Adding a new provider mints a fresh token only for the new one; existing tokens are unchanged. This avoids 401-ing running sandboxes when you re-run the wizard.

`--rotate-tokens` rolls every token:

```bash
oria egress setup --rotate-tokens
```
...
⚠  --rotate-tokens will invalidate proxy tokens in every running
   Oria sandbox.  They will start 401-ing against upstreams until restarted.
...
`oria egress setup` stops a running daemon when it rewrites config or token mappings, because the daemon keeps the old YAML in memory. After `--rotate-tokens`:

```bash
oria egress start
```

Containers already running hold the old tokens and will need to be restarted to pick up the new ones. New persistent Docker containers include an egress-posture label, so Oria will not reuse a pre-egress or pre-rotation container for new sessions.
...
If you set proxy-controlling env vars in your `docker_env:` config block (rare but possible), Oria refuses to start the sandbox when `enforce_on_docker: true` is set. This includes both:
...
The daemon's pidfile is written with `O_EXCL` + `O_NOFOLLOW` + ownership check. Concurrent `oria egress start` calls produce one of two outcomes:

- The existing pidfile points at a live iron-proxy → second start refuses with "another start in progress" + a hint to run `oria egress stop`
...
- **Binary not installed, `auto_install: true`** — first `oria egress setup` or `oria egress start` downloads it. SHA-256 verified against the upstream `checksums.txt`.
...
- **Port collision** — iron-proxy exits immediately; `oria egress start` reports the last 20 log lines and fails with non-zero exit.
...
```bash
oria egress setup --no-bitwarden
```
...
- Invalid `proxy.yaml` → run `oria egress setup` to regenerate
- CA cert / key permissions wrong → `chmod 0o600 ~/.hermes/proxy/ca.key`

### "iron-proxy did not bind \<bind-host\>:9090 within 5s"

The daemon started but never bound the listener. Usually means the binary is wedged or doing something expensive at startup. Check `~/.hermes/proxy/iron-proxy.log`. The orphan process is killed automatically and the pidfile cleaned up so you can just retry `oria egress start`.
...
If that times out while `oria egress status` shows `listening`, allow the bridge subnet in your firewall, e.g. for ufw:
...
Then `oria egress setup` (to regenerate `proxy.yaml`) and `oria egress stop && oria egress start`.
...
If the cert isn't there, check that `proxy.enabled: true` AND `oria egress status` shows `Listening yes`. If the env vars are missing, the sandbox image might be running an entrypoint that strips them — check your `docker_env` config.

### Sandbox sees `HTTP 401` from upstreams

Two common causes:

1. **Token-clobber on re-setup.** You ran `oria egress setup --rotate-tokens` (or rotated tokens some other way) and the running sandboxes still hold the old tokens. Restart the sandboxes.
...
The parent Oria process died during `oria egress start` (Ctrl-C during the listening probe, OOM, panic). The new fix-up logic writes the pidfile immediately after `Popen` so the orphan is recoverable:

```bash
oria egress stop     # finds the orphan via the pidfile, kills it
oria egress start
```

If `oria egress stop` says "iron-proxy was not running" but you can still see the daemon in `ps`, the pidfile got out of sync. Manual recovery:
...
- The CA is a 10-year self-signed cert on first generation. Rotation requires `openssl genrsa ...` by hand (or wait for a follow-up that adds `oria egress rotate-ca`).
- Re-running setup stops a running daemon after rewriting config or mappings; restart (or `oria egress reload` for ruleset-only changes) and restart already-running sandboxes after token rotation.
...
- Bitwarden integration: [`oria secrets bitwarden`](../secrets/bitwarden)
- Oria Docker terminal backend: [Docker](../docker)
