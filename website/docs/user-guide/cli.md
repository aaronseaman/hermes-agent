...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Oria terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Oria's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `oria setup --portal` — and you're ready to `oria chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Oria also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `oria --tui` — see the [TUI](tui.md) guide.
:::
...
oria chat --query-file prompt.txt
oria chat --query-file - < prompt.txt

# With a specific model
oria chat --model "anthropic/claude-sonnet-4"

# With a specific provider
oria chat --provider nous          # Use Nous Portal
oria chat --provider openrouter    # Force OpenRouter
...
oria chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
oria --continue               # Resume the most recent CLI session (-c)
oria --resume <session_id>    # Resume a specific session by ID (-r)
oria --resume latest          # Resume the most recent session (same as -c)
oria --resume latest --in ./dir    # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
oria chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
oria -w                           # Interactive mode in worktree
oria -w -z "Fix issue #123"       # Single query in worktree
```

### Worktree cleanup

`oria -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
launches `oria -w` for days — no longer accumulate merged scratch trees
...
The `oria plugins` commands manage native Oria plugins and portable Agent
...
oria plugins disable <plugin-name>
oria plugins update <plugin-name>
oria plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Oria currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Oria CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Oria CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `oria --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
On the `openai-codex` provider, `/usage` also shows any banked usage-limit resets on your ChatGPT account ("You have N resets banked - use /usage reset to activate"). `/usage reset` redeems one banked reset, fully restoring your 5-hour and weekly limits. Oria refuses to redeem while your limits aren't exhausted (a banked reset restores the full allowance, so spending it early wastes it) — pass `/usage reset --force` to redeem anyway.

### Session Resume Display

When resuming a previous session (`oria -c` or `oria --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Oria to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Oria supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Oria replies |
...
```bash
oria -s hermes-agent-dev,github-auth
oria chat -s github-pr-workflow -s github-auth
```

Oria loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
personalities:
  ...
  pirate: "Arrr! Ye be talkin' to Captain Oria..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Oria also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Oria — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Oria is working, Oria prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Oria to the background — just like any terminal process. The shell prints a confirmation:

```
Oria has been suspended. Run `fg` to bring Oria back.
```
...
Resume this session with:
  oria --resume 20260225_143052_a1b2c3
...
```bash
oria --continue                            # Resume the most recent CLI session
oria -c                                    # Short form
oria -c "my project"                       # Resume a named session (latest in lineage)
oria --resume 20260225_143052_a1b2c3       # Resume a specific session by ID
oria --resume "refactoring auth"           # Resume by title
oria --resume latest                       # Resume the most recent session (same as -c)
oria --resume latest --in ./my-project     # Latest session for ./my-project's workspace
oria -r 20260225_143052_a1b2c3             # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `oria sessions rename <id> <title>` from the command line. Use `oria sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Oria's SQLite state database under `~/.hermes/state.db`. The database keeps:
...
Oria immediately confirms the task and gives you back the prompt:
