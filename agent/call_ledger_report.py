...
def summarize(records: List[dict]) -> Dict[str, Any]:
    ...
    return {
        ...
        # Repeats are decided when a record is written, so two fingerprint kinds in one slice mean
        # the rates above span two definitions and are not comparable.
        "signature_kinds": sorted({str(t.get("signature_kind") or "unknown") for t in tools}),
...
def _format_summary(s: Dict[str, Any], indent: str = "  ") -> List[str]:
    ...
    rows = [
        ...
        ("Repeated signatures", f"{_fmt(s['repeated_signature_rate_turn'], 'pct')} within turn, "
                                ...
                                + (f" — MIXED fingerprint kinds {s['signature_kinds']}, not comparable"
                                   if len(s["signature_kinds"]) > 1 else "")),
