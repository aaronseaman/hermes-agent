...
class BitwardenSource(SecretSource):
    ...
    _AUTH_HINT = (
        "Run `oria secrets bitwarden token` to paste a fresh access "
        "token (create one in the Bitwarden web app: Secrets Manager → "
        "Machine accounts → Access tokens).  Wrong region?  Re-run "
        "`oria secrets bitwarden setup` and pick EU/self-hosted."
    )
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        if not access_token:
            return result.fail(f"secrets.bitwarden.enabled is true but {access_token_env} is "
                               "not set.  Run `oria secrets bitwarden setup`.", ErrorKind.NOT_CONFIGURED)
        project_id = str(cfg.get("project_id") or "")
        if not project_id:
            return result.fail("secrets.bitwarden.project_id is empty.  Run `oria secrets bitwarden setup`.",
        ...
        if binary is None:
            return result.fail("bws binary not available and auto-install is disabled.  "
                               "Run `oria secrets bitwarden setup` to install.", ErrorKind.BINARY_MISSING)
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `oria secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `oria secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `oria secrets bitwarden setup` to install."
        )
        return result
