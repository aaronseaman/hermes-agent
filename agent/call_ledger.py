...
# How the ``signature`` field was derived. A state-aware call fingerprint can replace it later
# without changing the record shape; the report only compares signatures of the same kind.
...
def end_turn(token, *, outcome: str) -> None:
    """Write the turn summary and release the binding. Waits briefly for queued records so
    short-lived processes (batch workers) do not exit with an unwritten tail."""
    if token is None:
        return
    try:
